; PROMETHEUS ZX Spectrum assembler/monitor
; Clean-room semantic reconstruction of the original machine image.
; Machine-emitting statements remain byte-identical to the historical TAP.
;
; ============================================================================
; SOURCE ORGANIZATION AND DESIGN NOTES
; ============================================================================
;
; Purpose of this file
; --------------------
;
; This is the complete resident PROMETHEUS program together with the small
; installation program that prepares it for execution.  The source is kept in
; one file because the historical image tightly interleaves executable code,
; writable operands, compact tables, strings, initial workspaces and installer
; data.  The includes near the installer and at the end contain generated or
; mechanically preserved tables; they are part of the same emitted image.
;
; The reconstruction has two simultaneous goals:
;
;   * explain the original program at routine, data-structure and algorithm
;     level; and
;   * preserve the exact historical bytes until an intentional functional
;     change is made.
;
; Comments therefore concentrate on intent, invariants, formats and unusual
; control flow.  Ordinary instructions are intentionally left uncommented when
; their purpose is already clear from surrounding labels and block comments.
;
; Physical source/image layout
; ----------------------------
;
; The major regions appear in this order:
;
;   1. Shared addresses, Spectrum ROM entry points, system variables and
;      program-wide constants.
;
;   2. Relocation-safe bootstrap, assembled for INSTALLATION_ADDRESS.  The CODE
;      block may actually be loaded elsewhere; this fragment discovers its
;      physical load address and copies the position-independent installer to
;      LOADER_ADDRESS ($5000).
;
;   3. Installer proper, logically assembled at $5000.  It displays the setup
;      screen, records installation options in writable instruction operands,
;      copies the requested resident image, applies configuration settings and
;      relocates absolute words before transferring control to the program.
;
;   4. Generated configuration-patch and relocation streams, followed by the
;      installer bitmap/logo tail.  The generated include files must not be
;      edited by hand; the build tools derive them from current source labels
;      and multi-origin probe assemblies.
;
;   5. Relocatable resident payload, beginning at ORG 0.  Linking the payload at
;      zero makes every internal absolute word contain an offset.  Installation
;      adds the chosen resident base to the words listed by the generated
;      relocation streams.
;
;   6. Monitor prefix, entered through ENTRY_POINT_WITH_MONITOR.  In the
;      historical image this prefix occupies exactly 5,000 bytes and contains
;      the front panel, memory tools, disassembler, breakpoint/trace machinery,
;      tape block operations and the saved user-processor image.
;
;   7. Assembler/editor body, beginning at ENTRY_POINT_WITHOUT_MONITOR.  This is
;      also the entry used by an assembler-only installation.  It contains the
;      command dispatcher, compressed-source editor, parser, two-pass assembler,
;      symbol management, code emission, tape import/export and shared display
;      and keyboard services.
;
;   8. Static language and UI tables, the included instruction table, initial
;      empty compressed source records and the initial symbol-table/code-end
;      tail.  relocatablePayloadEnd marks the emitted end without adding bytes.
;
; Installation and relocation model
; ---------------------------------
;
; The distributed CODE block is an installation image, not simply a resident
; binary fixed at one address.  Runtime installation proceeds as follows:
;
;   * recover the address at which the image was physically loaded;
;   * copy the installer to its fixed temporary workspace at $5000;
;   * let the user choose a destination and whether the monitor is included;
;   * copy either the complete payload or only the assembler/editor suffix;
;   * write the selected configuration bytes into labelled resident operands;
;   * add the installation base to every generated 16-bit relocation target;
;   * patch the two intentional assembler-only entry differences; and
;   * start the relocated program with a stack inside the installed image.
;
; Relocation metadata is discovered by assembling the payload at several
; artificial origins and comparing the emitted bytes.  A normal internal
; absolute address changes by the probe origin and is entered in the compact
; delta stream automatically.  Relative branches, self-relative table offsets,
; ROM addresses, screen addresses and numerical constants do not require
; runtime relocation.  Exceptional address-looking data is marked explicitly
; beside its source label and validated by the generator.
;
; Avoid introducing isolated LOW/HIGH address bytes: the historical relocator
; can adjust complete little-endian words only.  The build deliberately rejects
; origin-dependent bytes that are not covered by a generated 16-bit target.
;
; Resident architecture and coding principles
; -------------------------------------------
;
; PROMETHEUS is not organized around a single calling convention.  Register,
; flag and alternate-register-bank contracts are local to each routine and are
; documented where they matter.  Many hot paths pass state in registers or on
; the machine stack rather than through permanent memory.
;
; Self-modifying code is a fundamental storage technique, not an accidental
; side effect.  Immediate operands and occasionally opcodes hold persistent
; pointers, modes, callbacks and cached state.  Labels beginning with `varc`
; identify values stored directly in emitted code; code that updates them must
; preserve the exact operand width and instruction shape.
;
; Compact data encodings are used throughout:
;
;   * strings commonly terminate by setting bit 7 of their final character;
;   * many vectors store one-byte self-relative displacements;
;   * command, mnemonic and operand tables use packed descriptors and biased
;     counts rather than conventional records;
;   * source text is stored as variable-length compressed records and shares a
;     movable memory region with the symbol table;
;   * 16-bit values are little-endian unless a comment states otherwise; and
;   * some byte sequences deliberately resemble Z80 instructions even though
;     they are descriptor data.
;
; The Spectrum display is also working memory during installation, and several
; resident routines depend directly on the ROM keyboard, character, tape and
; pixel-address services listed below.  Interrupt state, stack placement and
; alternate register contents are therefore part of subsystem contracts even
; when they are not visible in an ordinary high-level call interface.
;
; Naming and label conventions
; ----------------------------
;
;   var...       writable variable or state cell
;   varc...      writable state embedded in an instruction operand or opcode
;   ROM_...      Spectrum ROM routine entry
;   SYSVAR_...   Spectrum system variable
;   ENTRY_...    externally meaningful resident entry/boundary
;   .name        private nearby control-flow label
;
; Dot-prefixed labels are used only for genuinely local control flow.  The
; assembler used by this project does not provide lexical local-label scopes,
; so these names remain globally unique even though the dot signals that they
; are not callable or address-taken interfaces.  Global semantic labels are
; retained for routines, shared continuation points, data, patch locations and
; section boundaries.
;
; Editing and verification rules
; ------------------------------
;
; Prefer symbolic expressions based on section labels over historical numeric
; offsets.  Do not reorder compact tables, merge apparently duplicate entries,
; replace opcode-shaped data, or simplify self-modifying sequences without first
; tracing every consumer.  A duplicated or unusual byte is often part of an
; index scheme, saved machine image, patch protocol or compatibility format.
;
; After any machine-emitting change, regenerate both metadata tables and run the
; complete release gate.  It checks deterministic generation, source annotation
; validity, relocation at several bases, full and assembler-only installation,
; execution to the editor loop, variable-length TAP construction and historical
; identity when no functional byte change was intended.
;
; ============================================================================
...
; =====================================================================
; RELOCATION-SAFE INSTALLATION BOOTSTRAP
; =====================================================================
;
; This first fragment executes at whatever address the CODE block was loaded
; to.  The manual explicitly allows loading the installation image at a range
; of addresses, so no absolute address inside this fragment may be trusted
; until the actual load address has been recovered.
;
; The bootstrap performs four jobs:
;
;   1. Disable interrupts and use the lower third of the display bitmap as
;      temporary workspace.  In particular, the screen is also used as a
;      private stack while the installer is active.
;   2. Recover the physical address at which this image was loaded.  It does
;      this without a Spectrum system variable: CALLing a ROM byte that is a
;      plain RET leaves a known return address on the machine stack.
;   3. Copy the small, position-independent installer to $5000.  All labels
;      between ORG $5000 and the relocation tables are therefore logical
;      runtime addresses, even though their bytes are stored later in this
;      file at the original load position.
;   4. Jump to the copied installer.  The much larger assembler/monitor body
;      is not moved until the user has selected the final installation
;      address and pressed ENTER.
;
; Byte-preservation note: all changes in this resurrection pass are labels
; and comments only.  The instruction stream below remains unchanged.
;
; Entry:
;   PC = physical address at which the complete installation image was loaded
;   SP = caller's stack
; Exit:
;   control transfers to installerEntryAt5000 at logical address $5000
;   interrupts remain disabled
; Clobbers:
;   AF, BC, DE, HL, SP; display bitmap $4000-$4FFF
; =====================================================================
bootstrapEntry:
    di                                      ; keep the self-relocating bootstrap non-interruptible
    ld hl,VRAM_ADDRESS
    ld de,VRAM_ADDRESS+1
    ld bc,00fffh
    ld (hl),l
    ldir                                    ; clear $4000-$4FFF for stack and installer use
    call ROM_ImmediateRET                   ; capture a return address tied to the actual load position
; The ROM RET has returned here.  Because CALL pushed the address following
; the CALL, the stack now contains a value tied to the actual load position.
; The two DEC SP instructions deliberately expose that return word again after
; the ROM RET has consumed it; POP BC then obtains the physical address just
; after the CALL.  Adding the compile-time negative displacement yields the
...
bootstrapRecoverLoadAddress:
    ...
    add hl,bc                               ; derive the physical start of the loaded image
    ld sp,VRAM_ADDRESS+0x20
    push hl
    ld hl,00017h                            ; physical offset of the copied installer segment in the load image
    add hl,bc
    ld bc,relocationSecondStreamTerminatorAndLogoStart+1-installerEntryAt5000
    ldir                                    ; copy the installer stub to its fixed $5000 execution address
    jp LOADER_ADDRESS
...
; =====================================================================
; INSTALLER PROPER, EXECUTING FROM $5000
; =====================================================================
;
; The preceding LDIR copied exactly $07CD bytes here.  At entry:
;
;   HL = first byte after the copied installer in the physical load image;
;        this is the beginning of the remaining logo data and then the main
;        relocatable payload.
;   DE = $5000 after the bootstrap's screen-clearing LDIR, then the copy.
;   SP = $4020, with the physical load base saved on the temporary stack.
;
; Two 20-character-wide bitmap rows are copied from the tail data into the
; Spectrum's non-linear display layout.  EX (SP),HL then exchanges:
;
;   HL <- original physical load base (used to display the detected address)
;   (SP) <- physical start of the assembler/monitor payload (used on ENTER)
;
; The installer is intentionally self-modifying.  Its current settings live
; as immediate operands or one-byte instructions, so no separate settings
; structure is needed in this very small temporary program.
; =====================================================================
installerEntryAt5000:
    ...
    ld de,02710h                            ; start formatting at the ten-thousands place
    ...
    call installerEmitDecimalDigit
; ---------------------------------------------------------------------
; Redraw installation screen and enter the keyboard loop
; ---------------------------------------------------------------------
;
; This is both the first display path and the common return address pushed by
; .installerDispatchKey.  Most key handlers simply RET; that RET lands here,
; so every accepted setting change is immediately reflected on screen.
;
; Strings are encoded inline after CALL installerPrintInlineString.  The high
; bit of the final character marks the end.  The printer consumes the return
; address as its source pointer and finally jumps to the byte after the string.
installerRedrawAndWait:
    ...
    ld hl,040c7h
    call installerPrintInlineString
    defb "(C) 1990 UNIVERSU",0xcd

    ld hl,048e2h
    call installerPrintInlineString
    defb "Press ENTER to run Assemble",0xf2

    ld hl,04883h                            ; position the installation-address prompt
    call installerPrintInlineString
    defb "Instalation address:"             ; encode the editable five-digit installation-address prompt and high-bit string terminator
installationAddressString:
    defb "00000_",0xa0

    ld hl,0484bh                            ; position the monitor option below the editable address
    call installerPrintInlineString
    defb "Monitor",0xba

; Self-modifying setting: immediate operand of LD A,n.
;
;   $4D (ASCII "M") = install assembler plus monitor
;   $00             = install assembler only
;
; M toggles the byte by XOR $4D.  Later, RRCA converts bit 0 of $4D into carry,
; so carry set selects the full 16,000-byte image and carry clear selects the
; 11,000-byte assembler-only image.
varcMonitorInstallFlag:
    ld a,04dh                               ; current monitor-install flag stored in this operand
    or a
    ld hl,04853h
    jr z,.installerPrintMonitorDisabled      ; show No when the monitor has been disabled
    call installerPrintInlineString
    defb "Ye",0xf3

    jr .installerInitializeAttributes
; The flag is zero: render "No ".  The trailing high-bit-marked space
; overwrites any third character left by the previously displayed "Yes".
.installerPrintMonitorDisabled:
    call installerPrintInlineString
    defb "No",0xa0

; ---------------------------------------------------------------------
; Build the installer colour layout in the Spectrum attribute file
; ---------------------------------------------------------------------
;
; The first four attribute rows belong to the logo.  The remaining regions
; use two self-modifying colour values:
;
;   varcInstallerTextAttribute+1      normal text
;   varcInstallerHighlightAttribute+1 selected/edit line
;
; The helper attributeCopyPaperColourToInk derives an invisible-text variant
; by copying PAPER bits into INK bits.  The single resulting cell hides the
; editing cursor area without changing its background colour.
.installerInitializeAttributes:
    ld hl,ATTRIBUTES_ADDRESS
    ld de,ATTRIBUTES_ADDRESS+1
    ld bc,00020h
    ld (hl),l
    ldir                                    ; fill the first logo attribute row
    ld c,020h
    ld (hl),006h
    ldir                                    ; fill the second logo attribute row
    ld c,020h
    ld (hl),046h
    ldir                                    ; fill the third logo attribute row
    ld c,0a0h
    ld (hl),007h
    ldir                                    ; fill the remaining logo attribute region
    ld bc,000e0h
; Self-modifying normal text attribute.  I, P and B alter selected bit
; fields through .installerAdjustMaskedSetting.  Initial value $38 means black
; ink on white paper, normal brightness.
varcInstallerTextAttribute:
    ld (hl),038h                            ; seed the normal text area from the configurable attribute byte
    ld a,(hl)
    ldir                                    ; fill the normal text region
; Self-modifying highlighted-line attribute.  CAPS SHIFT+I/P/B changes this
; byte instead of the normal text attribute.  Initial value $30 is black ink
; on yellow paper.
varcInstallerHighlightAttribute:
    ld (hl),030h
    ld bc,00020h
    ldir                                    ; copy it into the adjacent cell
    call attributeCopyPaperColourToInk      ; derive the border/status colour from PAPER
    ld b,001h
    ld (hl),a                               ; write the derived colour into the first cell
    ldir                                    ; copy it into the adjacent cell
; Self-modifying keyboard-click duration.  X increments and CAPS SHIFT+X
; decrements its lower six bits.  The value becomes the inner OUT ($FE),A loop
; count, so larger values produce a longer audible key echo.
varcKeyboardEchoDelay:
    ld e,00ah
    ld hl,0012ch
    ld a,(varcInstallerTextAttribute+1)     ; derive speaker/border bits from the selected PAPER colour
    rrca                                    ; rotate PAPER bits into the port-$FE layout
    rrca                                    ; rotate PAPER bits into the port-$FE layout
    rrca                                    ; rotate PAPER bits into the port-$FE layout
; Produce the configured keyboard echo.  A was derived from the PAPER colour;
; bit 4 is alternated to drive the Spectrum speaker through port $FE.  HL is a
; timing counter and E is the programmable inner-loop length.
.installerEchoToneOuterLoop:
    ld b,e
    xor 010h                                ; alternate the Spectrum speaker output bit
; One half-wave of the click.  OUT ($FE),A also writes border/microphone bits,
; but only the alternating speaker bit is significant here.
.installerEchoTonePulseLoop:
    out (0feh),a                            ; drive the speaker for this half-wave
    djnz .installerEchoTonePulseLoop
    dec hl
    inc h
    dec h
    jr nz,.installerEchoToneOuterLoop
    ld h,080h
; Short settling delay before polling the keyboard again.  INC H / DEC H is a
; flag-preserving timing idiom: DEC H supplies the zero test without changing
; the 16-bit counter value established by INC HL.
.installerEchoPauseLoop:
    inc hl
    inc h
    dec h                                   ; restore H and set Z only when the settling counter has wrapped to zero
    jr nz,.installerEchoPauseLoop            ; wait until the keyboard has settled
; ---------------------------------------------------------------------
; Wait for one Spectrum key and normalize shifted variants
; ---------------------------------------------------------------------
;
; ROM_KeyboardScanning waits for a stable matrix state; ROM_KeyboardTest then
; translates it.  The ROM leaves shift information in B.  INC B makes zero
; mean an unshifted character.  Shifted key codes are represented internally
; by adding $80, giving convenient constants such as:
;
;   CAPS SHIFT+I = $C9, CAPS SHIFT+P = $D0, CAPS SHIFT+B = $C2
;   DELETE       = $B0, CAPS SHIFT+X = $D8
;
; E retains the untranslated ROM character, although this dispatcher mostly
; works with the normalized value in A.
.installerWaitForKey:
    call ROM_KeyboardScanning
    jr nz,.installerWaitForKey               ; wait until the ROM reports a stable key state
    call ROM_KeyboardTest                   ; translate the matrix result to a character code
    jr nc,.installerWaitForKey               ; ignore scans that do not yield a usable key
    ld e,a
    inc b                                   ; convert the ROM shift indication into a zero/nonzero test
    jr z,.installerDispatchKey               ; dispatch an unshifted key directly
    add a,080h                              ; encode a shifted key in the installer’s internal range
; Push the redraw entry as a synthetic return address.  Every ordinary handler
; below ends in RET, which therefore redraws the complete installer screen and
; resumes key scanning.  ENTER is exceptional: it removes this return address
; and commits the installation.
.installerDispatchKey:
    ld hl,installerRedrawAndWait
    push hl
    cp 049h
    ld hl,varcInstallerTextAttribute+1      ; default colour edits to the normal text attribute
; Select masked update parameters for INK:
;   D = +1 step, E = $07 mask (attribute bits 0-2).
; The same entry is reused for CAPS SHIFT+I after HL has been redirected to the
; highlighted attribute byte.
.installerSelectInkAdjustment:
    ld de,00107h
    jr nz,.installerCheckPaperKey
; ---------------------------------------------------------------------
; Change one packed setting field without disturbing adjacent bits
; ---------------------------------------------------------------------
;
; Inputs:
;   HL -> byte to modify
;   D  = signed increment ($01, $08, $40, or $FF)
;   E  = bit mask selecting the field
;
; Calculation:
;   new = (old AND NOT mask) OR ((old + D) AND mask)
;
; This cycles INK, PAPER and BRIGHT in place and also wraps the six-bit echo
; duration.  Because the caller pre-pushed installerRedrawAndWait, RET both
; completes the update and refreshes the screen.
.installerAdjustMaskedSetting:
    ld a,e                                  ; derive the complement of the selected field mask
    cpl                                     ; preserve every packed field outside the selection
    and (hl)
    ld c,a                                  ; save those surrounding bits while cycling the field
    ld a,(hl)                               ; reload the complete packed setting
    add a,d
    and e                                   ; wrap the result inside the field mask
    or c
    ld (hl),a                               ; commit the new packed setting
    ret
; Test unshifted P.  Shifted P reaches the following selector directly after HL
; has been changed to the highlight attribute.
.installerCheckPaperKey:
    cp 050h
; PAPER occupies bits 3-5.  Adding $08 advances one paper colour while
; mask $38 keeps every other packed attribute bit unchanged.
.installerSelectPaperAdjustment:
    ld de,00838h
    jr z,.installerAdjustMaskedSetting       ; cycle PAPER when requested
    cp 042h                                 ; test the BRIGHT toggle key
; BRIGHT is bit 6.  Adding $40 under mask $40 toggles it; a second press
; wraps the selected field back to zero.
.installerSelectBrightnessAdjustment:
    ld de,04040h
    jr z,.installerAdjustMaskedSetting       ; toggle BRIGHT when requested
    ld hl,varcInstallerHighlightAttribute+1-installerEntryAt5000+LOADER_ADDRESS ; redirect shifted colour keys to the highlighted row
    cp 0c9h
    jr z,.installerSelectInkAdjustment       ; apply shifted I to the highlighted-row INK field
    cp 0d0h
    jr z,.installerSelectPaperAdjustment     ; apply shifted P to the highlighted-row PAPER field
    cp 0c2h
    jr z,.installerSelectBrightnessAdjustment ; apply shifted B to the highlighted-row BRIGHT bit
    cp 044h                                 ; test the bold-font toggle
    jr nz,.installerCheckMonitorKey
    ld hl,varcInstallerBoldTransform        ; select the self-modified bold opcode
    ld a,00fh
; Shared one-byte code toggle.
;
; D selects varcInstallerBoldTransform and A=$0F changes NOP ($00) to RRCA
; ($0F), or back again.  M selects varcMonitorInstallFlag+1 and A=$4D changes
; the immediate setting byte between $4D and zero.
.installerXorToggleByte:
    xor (hl)                                ; toggle the selected option byte
    ld (hl),a
    ret
; Continue dispatch after D.  The order deliberately loads the address of the
; monitor flag before testing M so the same XOR helper can be reached directly.
.installerCheckMonitorKey:
    cp 04dh                                 ; test the monitor-install toggle
    ld hl,varcMonitorInstallFlag+1-installerEntryAt5000+LOADER_ADDRESS ; select the monitor flag operand
    jr z,.installerXorToggleByte             ; toggle monitor inclusion when M was pressed
    cp 058h
    ld hl,varcKeyboardEchoDelay+1-installerEntryAt5000+LOADER_ADDRESS ; select the stored click-duration operand
    ld de,0013fh                            ; choose +1 within the six-bit echo field
    jr z,.installerAdjustMaskedSetting       ; apply the selected echo-delay adjustment
    ld d,0ffh                               ; change the same field to a −1 step for shifted X
    cp 0d8h
    jr z,.installerAdjustMaskedSetting       ; apply the selected echo-delay adjustment
    cp 043h
    jr nz,varcInstallationAddressCursor     ; leave case-mode handling and continue with address/ENTER dispatch when C was not pressed
; Self-modifying case-mode index stored in the immediate operand of LD A,n.
; C cycles 0 -> 1 -> 2 -> 0.  Doubling the index selects one two-byte Z80
; instruction from installerCaseModeInstructionTable.
varcInstallerCaseMode:
    ld a,000h                               ; current case-mode index from this operand
    inc a
    cp 003h                                 ; detect wrap after the third policy
    jr nz,.installerApplyCaseMode            ; keep the incremented mode when it is still within range
    xor a                                   ; wrap the mode index back to mixed case
; Store the new mode index, fetch the corresponding complete two-byte
; instruction, and overwrite varcInstallerCaseTransform.  The three modes are
; normal mixed case, forced lower case, and forced upper case.
.installerApplyCaseMode:
    ld (varcInstallerCaseMode+1),a          ; persist the new mode in the immediate operand
    add a,a                                 ; convert the mode index to a two-byte table offset
    ld hl,installerCaseModeInstructionTable-installerEntryAt5000+LOADER_ADDRESS
    ld b,000h                               ; form a 16-bit table index in BC
    ld c,a                                  ; place the doubled mode index in BC
    add hl,bc
    ld e,(hl)
    inc hl
    ld d,(hl)
    ex de,hl
    ld (varcInstallerCaseTransform-installerEntryAt5000+LOADER_ADDRESS),hl ; patch the inline-string case-conversion policy
    ret
; Executable two-byte templates copied verbatim into the inline-string renderer:
;   AND $FF = preserve source character
;   OR  $20 = force alphabetic character to lower case
;   AND $DF = force alphabetic character to upper case
installerCaseModeInstructionTable:
    and 0ffh                                ; encode the two-byte mixed-case template that leaves each alphabetic character unchanged
    or 020h                                 ; encode the two-byte lower-case template copied into the installer string renderer
    and 0dfh                                ; encode the two-byte upper-case template copied into the installer string renderer
; Self-modifying cursor pointer.  The immediate operand of LD HL,nn points at
; the underscore in the five-character decimal address field.  DELETE moves
; it left; a digit overwrites the underscore and moves it right.  The pointer
; itself is rewritten by .installerPlaceAddressCursor.
varcInstallationAddressCursor:
    ld hl,installationAddressString+5-installerEntryAt5000+LOADER_ADDRESS
    cp 0b0h
    jr nz,.installerCheckAddressDigit
    dec hl
    ld a,(hl)
    cp 03ah
    ret z                                   ; refuse to move DELETE left across the ':' delimiter preceding the address field
    inc hl
    ld (hl),020h                            ; erase the old cursor cell
    dec hl
; Write the visible underscore at HL and remember HL as the current editing
; position by patching varcInstallationAddressCursor+1.
.installerPlaceAddressCursor:
    ld (hl),05fh                            ; draw the underscore cursor at the new position
    ld (varcInstallationAddressCursor+1-installerEntryAt5000+LOADER_ADDRESS),hl ; remember the cursor location in the self-modified operand
    ret
; Accept only ASCII '0'..'9'.  Looking one byte beyond the cursor for bit 7
; prevents entry past the fixed five-digit field: the byte after the field is
; the high-bit-terminated trailing space used by the inline string.
.installerCheckAddressDigit:
    cp 030h
    jr c,.installerCheckEnterKey             ; reject a non-digit below ASCII zero
    cp 03ah
    jr nc,.installerCheckEnterKey            ; reject a non-digit above ASCII nine
    inc hl
    bit 7,(hl)                              ; detect the high-bit terminator after the fifth digit
    ret nz                                  ; refuse a sixth digit when the cursor has reached the high-bit string terminator
    dec hl
    ld (hl),a
    inc hl
    jr .installerPlaceAddressCursor
; ---------------------------------------------------------------------
; ENTER: commit selected settings into the relocatable payload
; ---------------------------------------------------------------------
;
; The synthetic redraw return address is discarded.  The next stack word is
; the physical payload pointer saved by EX (SP),HL at installer entry; it is
; kept on the stack for the later copy operation.
;
; The ordinary screen stack is temporarily replaced by a 14-word signed delta
; stream at installerConfigurationPatchDeltas.  Each call to
; installerAdvancePatchPointer pops its own return address, pops the next
; signed delta, adds that delta to HL, then restores the return address.  This
; compact mechanism visits widely separated configuration bytes in the main
; image without storing fourteen absolute addresses.
;
; Confirmed patch classes include font thickness, case-conversion opcode,
; keyboard echo length, normal/highlight attributes, hidden edit-line colour,
; and border colour.  Exact destinations are marked by ordered
; @config-patch labels and listed in the generated configuration report.
.installerCheckEnterKey:
    cp 00dh
    ret nz                                  ; ignore every remaining unrecognized key through redraw
    pop af                                  ; discard the synthetic redraw return address
    pop hl                                  ; recover the physical resident-payload pointer
    push hl                                 ; preserve that payload pointer for the final image move
    ld (varcInstallerSavedStackPointer+1-installerEntryAt5000+LOADER_ADDRESS),sp ; remember the private installer stack
    ...
    dec hl
    call installerAdvancePatchPointer
    ld a,(varcKeyboardEchoDelay+1-installerEntryAt5000+LOADER_ADDRESS) ; copy the selected keyboard-click duration
    call installerStoreAAndAdvancePatchPointer
    ld a,(varcInstallerTextAttribute+1)     ; copy the selected normal text attribute
    call installerStoreAAndAdvancePatchPointer
    call installerStoreAAndAdvancePatchPointer
    call installerStoreAAndAdvancePatchPointer
    xor 001h                                ; derive the front-panel editor colour variant
    call installerStoreAAndAdvancePatchPointer
    call installerStoreAAndAdvancePatchPointer
    call attributeCopyPaperColourToInk      ; derive the border colour from the selected PAPER bits
    call installerStoreAAndAdvancePatchPointer
    and 007h                                ; retain only the three ULA border-colour bits derived from the selected normal PAPER colour
    call installerStoreAAndAdvancePatchPointer
    call installerStoreAAndAdvancePatchPointer
    ld a,(varcInstallerHighlightAttribute+1-installerEntryAt5000+LOADER_ADDRESS) ; selected highlight attribute for the final resident highlight-colour patches
    call installerStoreAAndAdvancePatchPointer
    call installerStoreAAndAdvancePatchPointer
    ld (hl),a
; Restore the temporary display-memory stack after the delta stream is
; exhausted.  ENTER patched this LD SP,nn operand with the live SP value.
varcInstallerSavedStackPointer:
    ld sp,00000h
    ld hl,00000h                            ; initialize the parsed installation destination accumulator to zero
    ld de,installationAddressString
; Convert the displayed decimal installation address into HL.
;
; The multiply-by-ten sequence is:
;   old*2, save it, old*4, old*8, add saved old*2 -> old*10
; Then the numeric value of the current ASCII digit is added.  Parsing stops
; at the underscore, which remains immediately after the last entered digit.
.installerParseAddressDigit:
    ld a,(de)
    inc de
    cp 05fh
    jr z,.installerAddressParsed             ; finish when the five-digit field terminator is reached
    add hl,hl                               ; form twice the previous destination
    push hl
    add hl,hl
    add hl,hl
    pop bc
    add hl,bc
    sub 030h                                ; convert the ASCII digit to its numeric value
    ld c,a
    ld b,000h                               ; zero-extend the numeric digit before adding it to the address accumulator
    add hl,bc
    jr .installerParseAddressDigit
; At this point HL is the requested destination.  POP DE retrieves the physical
; source-payload address saved at installer entry; EX DE,HL leaves:
;
;   HL = source payload
;   DE = requested destination
;
; The monitor flag is rotated into carry and preserved across the copy so the
; later relocation path knows which image layout was selected.
.installerAddressParsed:
    pop de                                  ; recover the physical resident-payload pointer retained on the installer stack
    ex de,hl                                ; exchange registers so HL is the physical source payload and DE the requested destination
    ld a,(varcMonitorInstallFlag+1-installerEntryAt5000+LOADER_ADDRESS) ; chosen monitor-install setting
    rrca
    push af                                 ; preserve monitor-inclusion carry across image selection, copying, and relocation
    ld bc,relocatablePayloadEnd-ENTRY_POINT_WITH_MONITOR
    jr c,.installerCopyImage                 ; keep the full 16,000-byte monitor-plus-assembler image when monitor inclusion is enabled
; Assembler-only installation omits the first 5,000 payload bytes occupied by
; the monitor.  Before skipping them, one payload word is increased by the
; same $1388 displacement; this adjusts a monitor-dependent reference in the
; retained assembler image.  The source pointer then advances by 5,000 and
; the copy length becomes 11,000 bytes ($2AF8), rather than 16,000 ($3E80).
installerPrepareAssemblerOnlyImage:
    push hl                                 ; preserve the physical payload source while patching its assembler-only reference
    push de
...
; Copy the chosen payload safely even when source and destination overlap.
; Comparing source HL with destination DE chooses the same rule as memmove:
; copy forward with LDIR when safe, otherwise move both pointers to the ends
; of their blocks and copy backward with LDDR.
.installerCopyImage:
    push de
    push hl                                 ; preserve the source pointer while comparing source and destination ordering
    xor a
    sbc hl,de
    pop hl
    jr c,.installerCopyImageBackward         ; copy backward when the destination overlaps above the source
    ldir                                    ; move the resident image forward
    jr .installerRelocateCopiedImage
; Destination lies above source inside the source range, so forward copying
; could overwrite unread bytes.  Convert both start pointers to inclusive end
; pointers and use LDDR.
.installerCopyImageBackward:
    ...
    dec hl
    ex de,hl
    lddr                                    ; move the overlapping resident image backward
; ---------------------------------------------------------------------
; Apply compact relocation streams to the copied image
; ---------------------------------------------------------------------
;
; The original destination address was saved on the machine stack before the
; copy.  It becomes DE, the running relocation target pointer.  BC is retained
; as an installation-dependent additive base.  Two relocation streams cover
; different portions/layouts of the image; the full-system path applies both,
; while assembler-only installation adjusts the bases before entering the
; shared second stream.
;
; Some arithmetic here is layout-specific and intentionally remains described
; conservatively.  The stream decoder itself is fully characterized below.
.installerRelocateCopiedImage:
    pop bc
    ld d,b
    ld e,c
    pop af
    push bc                                 ; retain the installed base for the final entry transfer
    push af                                 ; retain the image-selection flags across the relocation-table decoder
    jr nc,.installerPrepareAssemblerOnlyRelocation ; skip monitor relocation entries for assembler-only installation
    ld hl,vr_l05353h
    call installerApplyRelocationTable
    ld hl,ENTRY_POINT_WITHOUT_MONITOR-ENTRY_POINT_WITH_MONITOR-RELOCATION_FIRST_STREAM_FINAL_TARGET_OFFSET
                                            ; bridge from the first stream's final target to the assembler section
    add hl,de
    ex de,hl
    jr .installerApplySecondRelocationTable
; Assembler-only relocation path.  The fixed value $EC78 is added to the saved
; destination/base value to form the additive relocation base expected by the
; second stream after the monitor prefix has been omitted.
.installerPrepareAssemblerOnlyRelocation:
    ld hl,ENTRY_POINT_WITH_MONITOR-ENTRY_POINT_WITHOUT_MONITOR
                                            ; compensate for omitting the monitor prefix from the linked payload
    add hl,bc
    ld b,h
    ld c,l
; Apply the shared second relocation stream.  Afterwards HL is formed from DE
; plus $F136; in the assembler-only case the saved base word is written there.
; In the full-system case carry bypasses that write because the corresponding
; layout was already handled by the first stream.
.installerApplySecondRelocationTable:
    ld hl,vr_l05547h
    call installerApplyRelocationTable
    ld hl,assemblerOnlyMonitorFallbackAddress-ENTRY_POINT_WITH_MONITOR-RELOCATION_SECOND_STREAM_FINAL_TARGET_OFFSET
    add hl,de
    pop af
    pop bc
    jr c,.installerStartRelocatedProgram     ; skip the assembler-only base-word patch when the full monitor layout is installed
    ld (hl),c
    inc hl
    ld (hl),b
; Build a fresh stack inside the installed image at destination+$0104.
; Pushing BC followed by RET transfers control to the relocated entry address
; encoded in BC.  This is why the bootstrap's initial DI remains in force: the
; installed program takes responsibility for establishing its own runtime
; interrupt/stack state.
.installerStartRelocatedProgram:
    ld hl,00104h                            ; select the private stack offset inside the newly installed resident image
    add hl,de
    ld sp,hl
    push bc
    ret
...
; Store A at the current configuration destination and fall through to the
; delta-stream pointer advance routine.
installerStoreAAndAdvancePatchPointer:
    ld (hl),a
; Advance HL by the next signed 16-bit delta stored on the temporary stack.
;
; Stack on entry: return-address, delta, remaining-deltas...
; POP BC preserves the return address, POP DE obtains the delta, ADD HL,DE
; advances the destination, and PUSH BC restores normal CALL/RET behaviour.
installerAdvancePatchPointer:
    pop bc                                  ; remove and preserve this helper's return address
    pop de                                  ; consume the next signed patch delta from the temporary stack
    add hl,de                               ; advance the current resident patch target by the next signed delta
    push bc                                 ; restore this helper's return address above the remaining delta stream
    ret                                     ; resume the compact patch sequence


; Emit one decimal digit of the detected physical load address.
;
; Inputs:
;   HL = remaining unsigned value
;   DE = current decimal divisor (10000, 1000, 100, 10, or 1)
;   BC = destination character pointer
; Output:
;   one ASCII digit stored at (BC), BC advanced, HL reduced to remainder
;
; Repeated subtraction starts A at '/' so the first successful increment is
; ASCII '0'.  The failing subtraction is repaired by ADD HL,DE.
installerEmitDecimalDigit:
    ld a,02fh                               ; seed the digit counter one below ASCII '0' for repeated-subtraction formatting
; Count how often the divisor fits into the remaining value.
...
; Fourteen generated little-endian signed displacements used as a stack by
; the ENTER configuration copier.  Each destination is marked in the payload
; by an ordered configuration-patch label, so edits that move code regenerate this
; walk instead of relying on historical absolute addresses.
installerConfigurationPatchDeltas:
include "configurationPatchTable.asm"


; Convert an attribute into a same-colour INK/PAPER attribute:
;
;   result = (attribute AND $F8) OR ((attribute AND $38) >> 3)
;
; PAPER/BRIGHT are retained and PAPER is copied into INK.  Text drawn with the
; resulting attribute is invisible against its own background.  Flags are not
; part of the interface; A and B are clobbered.
attributeCopyPaperColourToInk:
    and 0f8h
    ld b,a                                  ; retain those unshifted attribute bits while PAPER is rotated down into INK
    rrca
    rrca
    rrca
    and 007h                                ; isolate the rotated three-bit PAPER colour now occupying the INK field
    or b
    ret


; ---------------------------------------------------------------------
; Draw a high-bit-terminated inline string using the Spectrum ROM font
; ---------------------------------------------------------------------
;
; Call form:
;       ld hl,bitmapDestination
;       call installerPrintInlineString
;
;       ; execution resumes here
;
; Input:
;   HL = bitmap address of first character cell
; Hidden input:
;   the CALL return address points at the first inline character
; Output:
;   control jumps to the byte following the marked final character
;
; The destination is held in the immediate operand at
; varcInstallerStringDestination.  EXX separates the source-string pointer
; from glyph-rendering registers.  Alphabetic characters pass through the
; two-byte instruction patched by C.  The ROM glyph address is calculated as
; $3C00 + character*8.  D selects normal or bold row transformation.
installerPrintInlineString:
    ...
    ld (varcInstallerStringDestination+1-installerEntryAt5000+LOADER_ADDRESS),hl ; remember the current screen destination
    pop hl                                  ; take the CALL return address as the first byte of the inline high-bit-terminated string
; Fetch one source byte.  Bit 7 is removed for glyph lookup, but the original
; byte is fetched again after rendering so RLCA can test whether it marked the
; final character.
.installerPrintNextCharacter:
    ld a,(hl)
    and 07fh                                ; strip the inline-string terminator bit
    exx
    cp 041h
    jr c,.installerLocateRomGlyph            ; bypass case conversion for characters below uppercase A
    cp 05bh
    jr c,varcInstallerCaseTransform         ; apply the selected case transform to uppercase A-Z
    cp 061h
    jr c,.installerLocateRomGlyph
; Self-modifying two-byte alphabetic case transform.  C replaces this entire
; instruction with one entry from installerCaseModeInstructionTable.
varcInstallerCaseTransform:
    ...
    and 0ffh
; Convert character code A to its eight-byte ROM font address.  Setting H=$0F
; before two ADD HL,HL operations yields $3C00 + A*8, the Spectrum character
; generator area for printable characters.
.installerLocateRomGlyph:
    add a,a                                 ; double the character code as the first stage of eight-byte glyph indexing
    ld h,00fh                               ; seed the high byte so two further doublings produce ROM font base $3C00
    ld l,a
    add hl,hl                               ; scale the glyph index toward eight bytes per character
    add hl,hl                               ; scale the glyph index toward eight bytes per character
; Self-modifying destination pointer for the current character.  After one glyph
; is rendered, the low byte is incremented and written back into this LD DE,nn
; operand.  Installer strings do not cross a 256-byte bitmap boundary.
varcInstallerStringDestination:
    ld de,04000h
    push de                                 ; preserve the top-row destination while D advances vertically
    ld b,008h
; Render eight bitmap rows.  Incrementing D rather than DE follows the Spectrum
; display's $0100 stride between successive pixel rows within one character
; cell.  The source ROM glyph pointer advances linearly.
.installerRenderGlyphRow:
    ld a,(hl)
; Self-modifying one-byte font transform:
;
;   NOP  ($00): A remains the original glyph row; OR (HL) is idempotent.
;   RRCA ($0F): rotate the row one pixel right, then OR with the original row,
;               producing a two-pixel-thick version.
;
; D toggles this opcode with XOR $0F.  ENTER copies the selected opcode into
; the main assembler/monitor renderer.
varcInstallerBoldTransform:
    ...
    nop                                     ; optionally rotate the row when bold mode patches this to RRCA
    or (hl)                                 ; merge the original row to form normal or bold pixels
    ld (de),a
    inc hl
    inc d
    djnz .installerRenderGlyphRow
    pop hl
    inc l
    ld (varcInstallerStringDestination+1-installerEntryAt5000+LOADER_ADDRESS),hl ; remember the next character destination
    exx
    ld a,(hl)                               ; reload the encoded character to inspect its terminator bit
    inc hl
    rlca
...
; ---------------------------------------------------------------------
; Decode and apply one compact relocation stream
; ---------------------------------------------------------------------
;
; Inputs:
;   HL -> relocation stream
;   DE = running address inside copied image
;   BC = 16-bit relocation addend
;
; Stream format, established directly from this decoder:
;
;   $00             end of stream
;   $01-$C7         one relocation: the byte is the unsigned distance from
;                   the previous target to the next 16-bit word
;   $C8-$FF, count  repeated relocation: distance = prefix-$C8; the following
;                   byte is the number of equally spaced words to relocate
;
; The decoder first writes 1 over the prefix byte.  For the short form, HL
; still points to that rewritten byte, so exactly one relocation is performed.
; For the repeated form, HL advances to the explicit count byte, which is
; decremented in place until the run is complete.  A retains the decoded
; distance and is added to DE before every word in the run.
;
; At each target, the little-endian word is loaded, BC is added, and the word
; is written back.  Thus a sequence such as $CF,$1D means 29 relocation words
; separated by seven bytes each.  This description accounts for every branch
; and mutation in the decoder; later passes can correlate individual stream
; records with the operands they relocate.
installerApplyRelocationTable:
    ld a,(hl)
    or a
    ret z
    ld (hl),001h                            ; turn a short prefix into a synthetic one-item count
    cp 0c8h
    jr c,.installerApplyRelocationRun
    sub 0c8h                                ; decode the repeated-run distance
    inc hl
; Advance the running target pointer by A, preserving carry into D, then
; relocate the word at the resulting address.  At this point (HL) is either
; the synthetic count 1 written over a short-form prefix, or the explicit
; count byte following a repeated-form prefix.
.installerApplyRelocationRun:
    push af
    add a,e
    ld e,a
    jr nc,.installerRelocateWord
    inc d
; Add BC to the little-endian word at the current DE target.  Register exchanges
; allow ADD HL,BC to perform the 16-bit arithmetic while preserving the stream
; pointer on the machine stack.
.installerRelocateWord:
    ...
    ex de,hl
    add hl,bc
    ex de,hl                                ; return the rebased word to DE for storage
    ld (hl),d
    dec hl
    ld (hl),e
    ex de,hl
...
; ---------------------------------------------------------------------
; Copy one 20-character-wide, 8-pixel-high logo bitmap row
; ---------------------------------------------------------------------
;
; Inputs:
;   HL -> 20*8 linear source bytes, grouped by character column
;   DE = Spectrum bitmap address of top row, first column
; Output:
;   HL advanced by 160 bytes; DE advanced by 20 columns
;
; For each character column, eight source bytes are written by incrementing D,
; which moves down one Spectrum pixel row ($0100).  Restoring DE and incrementing
; E then moves one byte to the right.  This matches the Spectrum's interleaved
; bitmap layout without a lookup table.
installerDrawLogoRow:
    ld c,014h
.installerDrawNextLogoColumn:
    ld b,008h
    push de
; Copy the eight vertically separated bytes of the current column.
...
; The first zero byte terminates the second relocation stream and is also
; the first byte of the historical logo bitmap.  The installer copies it
; together with the generated table before using the remaining 320 bytes.
...
; BLOCK 'logo_image' (start 0x65b3 end 0x66f4)
    ...
    defb 0f0h                               ; encode the final byte of the installer logo bitmap tail
