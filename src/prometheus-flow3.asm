...
; command line: z80dasm -a -t -l -g 24000 -b blocks.txt prometheus.bin
    ...
    di                         ;5dc0    f3      .                  (flow from: 34bb 52ad)
    ld hl,04000h               ;5dc1    21 00 40    ! . @          (flow from: 5dc0)
    ld de,04001h               ;5dc4    11 01 40    . . @          (flow from: 5dc1)
    ld bc,00fffh               ;5dc7    01 ff 0f    . . .          (flow from: 5dc4)
    ld (hl),l                  ;5dca    75      u                  (flow from: 5dc7)
    ldir                       ;5dcb    ed b0   . .                (flow from: 5dca 5dcb)
    call 00052h                ;5dcd    cd 52 00    . R .          (flow from: 5dcb)
    dec sp                     ;5dd0    3b                     ;   (flow from: 0052)
    dec sp                     ;5dd1    3b                     ;   (flow from: 5dd0)
    pop bc                     ;5dd2    c1      .                  (flow from: 5dd1)
    ld hl,0fff0h               ;5dd3    21 f0 ff    ! . .          (flow from: 5dd2)
    add hl,bc                  ;5dd6    09      .                  (flow from: 5dd3)
    ld sp,04020h               ;5dd7    31 20 40    1   @          (flow from: 5dd6)
    push hl                    ;5dda    e5      .                  (flow from: 5dd7)
    ld hl,00017h               ;5ddb    21 17 00    ! . .          (flow from: 5dda)
    add hl,bc                  ;5dde    09      .                  (flow from: 5ddb)
    ld bc,007cdh               ;5ddf    01 cd 07    . . .          (flow from: 5dde)
    ldir                       ;5de2    ed b0   . .                (flow from: 5ddf 5de2)
    jp 05000h                  ;5de4    c3 00 50    . . P          (flow from: 5de2)
    ld de,04026h               ;5de7    11 26 40    . & @          (ghost flow from: 5de4)
    call 05342h                ;5dea    cd 42 53    . B S          (ghost flow from: 5000)
    ld e,046h                  ;5ded    1e 46   . F                (ghost flow from: 5352)
    call 05342h                ;5def    cd 42 53    . B S          (ghost flow from: 5006)
    ex (sp),hl                 ;5df2    e3      .                  (ghost flow from: 5352)
    ld bc,050a1h               ;5df3    01 a1 50    . . P          (ghost flow from: 500b)
    ld de,02710h               ;5df6    11 10 27    . . '          (ghost flow from: 500c)
    call 052b4h                ;5df9    cd b4 52    . . R          (ghost flow from: 500f)
    ld de,003e8h               ;5dfc    11 e8 03    . . .          (ghost flow from: 52bf)
    call 052b4h                ;5dff    cd b4 52    . . R          (ghost flow from: 5015)
    ld de,00064h               ;5e02    11 64 00    . d .          (ghost flow from: 52bf)
    call 052b4h                ;5e05    cd b4 52    . . R          (ghost flow from: 501b)
    ld e,00ah                  ;5e08    1e 0a   . .                (ghost flow from: 52bf)
    call 052b4h                ;5e0a    cd b4 52    . . R          (ghost flow from: 5021)
    ld e,001h                  ;5e0d    1e 01   . .                (ghost flow from: 52bf)
    call 052b4h                ;5e0f    cd b4 52    . . R          (ghost flow from: 5026)
    ld hl,04082h               ;5e12    21 82 40    ! . @          (ghost flow from: 52bf)
    call 052e6h                ;5e15    cd e6 52    . . R          (ghost flow from: 502b)
...
intro1_string_start:
    ...
    defb 0f2h                  ;5e33    f2      . 
intro1_string_end:
    ld hl,040c7h               ;5e34    21 c7 40    ! . @          (ghost flow from: 531b)
    call 052e6h                ;5e37    cd e6 52    . . R          (ghost flow from: 504d)
...
intro2_string_start:
    ...
    defb 0cdh                  ;5e4b    cd      . 
intro2_string_end:
    ld hl,048e2h               ;5e4c    21 e2 48    ! . H          (ghost flow from: 531b)
    call 052e6h                ;5e4f    cd e6 52    . . R          (ghost flow from: 5065)
...
intro3_string_start:
    ...
    defb 0f2h                  ;5e6d    f2      . 
intro3_string_end:
    ld hl,04883h               ;5e6e    21 83 48    ! . H          (ghost flow from: 531b)
    call 052e6h                ;5e71    cd e6 52    . . R          (ghost flow from: 5087)
...
intro4_string_start:
    ...
    defb 0a0h                  ;5e8e    a0      . 
intro4_string_end:
    ld hl,0484bh               ;5e8f    21 4b 48    ! K H          (ghost flow from: 531b)
    call 052e6h                ;5e92    cd e6 52    . . R          (ghost flow from: 50a8)
...
intro5_string_start:
    ...
    defb 0bah                  ;5e9c    ba      . 
intro5_string_end:
    ld a,04dh                  ;5e9d    3e 4d   > M                (ghost flow from: 531b)
    or a                       ;5e9f    b7      .                  (ghost flow from: 50b6)
    ld hl,04853h               ;5ea0    21 53 48    ! S H          (ghost flow from: 50b8)
    jr z,l5eadh                ;5ea3    28 08   ( .                (ghost flow from: 50b9)
    call 052e6h                ;5ea5    cd e6 52    . . R          (ghost flow from: 50bc)
...
intro_yes_string_end:
    jr intro_no_string_end                                     ;5eab    18 06   . .                                                    (ghost flow from: 531b)
l5eadh:
...
intro_no_string_start:
    ...
    defb 0a0h                  ;5eb2    a0      . 
intro_no_string_end:
    ld hl,05800h               ;5eb3    21 00 58    ! . X          (ghost flow from: 50c4)
    ld de,05801h               ;5eb6    11 01 58    . . X          (ghost flow from: 50cc)
    ld bc,00020h               ;5eb9    01 20 00    .   .          (ghost flow from: 50cf)
    ld (hl),l                  ;5ebc    75      u                  (ghost flow from: 50d2)
    ldir                       ;5ebd    ed b0   . .                (ghost flow from: 50d5 50d6)
    ld c,020h                  ;5ebf    0e 20   .                  (ghost flow from: 50d6)
    ld (hl),006h               ;5ec1    36 06   6 .                (ghost flow from: 50d8)
    ldir                       ;5ec3    ed b0   . .                (ghost flow from: 50da 50dc)
    ld c,020h                  ;5ec5    0e 20   .                  (ghost flow from: 50dc)
    ld (hl),046h               ;5ec7    36 46   6 F                (ghost flow from: 50de)
    ldir                       ;5ec9    ed b0   . .                (ghost flow from: 50e0 50e2)
    ld c,0a0h                  ;5ecb    0e a0   . .                (ghost flow from: 50e2)
    ld (hl),007h               ;5ecd    36 07   6 .                (ghost flow from: 50e4)
    ldir                       ;5ecf    ed b0   . .                (ghost flow from: 50e6 50e8)
    ld bc,000e0h               ;5ed1    01 e0 00    . . .          (ghost flow from: 50e8)
    ld (hl),038h               ;5ed4    36 38   6 8                (ghost flow from: 50ea)
    ld a,(hl)                  ;5ed6    7e      ~                  (ghost flow from: 50ed)
    ldir                       ;5ed7    ed b0   . .                (ghost flow from: 50ef 50f0)
    ld (hl),030h               ;5ed9    36 30   6 0                (ghost flow from: 50f0)
    ld bc,00020h               ;5edb    01 20 00    .   .          (ghost flow from: 50f2)
    ldir                       ;5ede    ed b0   . .                (ghost flow from: 50f4 50f7)
    call 052dch                ;5ee0    cd dc 52    . . R          (ghost flow from: 50f7)
    ld b,001h                  ;5ee3    06 01   . .                (ghost flow from: 52e5)
    ld (hl),a                  ;5ee5    77      w                  (ghost flow from: 50fc)
    ldir                       ;5ee6    ed b0   . .                (ghost flow from: 50fe 50ff)
    ld e,00ah                  ;5ee8    1e 0a   . .                (ghost flow from: 50ff)
    ld hl,0012ch               ;5eea    21 2c 01    ! , .          (ghost flow from: 5101)
    ld a,(050eeh)              ;5eed    3a ee 50    : . P          (ghost flow from: 5103)
    rrca                       ;5ef0    0f      .                  (ghost flow from: 5106)
    rrca                       ;5ef1    0f      .                  (ghost flow from: 5109)
    rrca                       ;5ef2    0f      .                  (ghost flow from: 510a)
l5ef3h:
    ld b,e                     ;5ef3    43      C                  (ghost flow from: 510b 5116)
    xor 010h                   ;5ef4    ee 10   . .                (ghost flow from: 510c)
l5ef6h:
    out (0feh),a               ;5ef6    d3 fe   . .                (ghost flow from: 510d 5111)
    djnz l5ef6h                ;5ef8    10 fc   . .                (ghost flow from: 510f)
    dec hl                     ;5efa    2b      +                  (ghost flow from: 5111)
    inc h                      ;5efb    24      $                  (ghost flow from: 5113)
    dec h                      ;5efc    25      %                  (ghost flow from: 5114)
    jr nz,l5ef3h               ;5efd    20 f4     .                (ghost flow from: 5115)
    ld h,080h                  ;5eff    26 80   & .                (ghost flow from: 5116)
l5f01h:
    inc hl                     ;5f01    23      #                  (ghost flow from: 5118 511d)
    inc h                      ;5f02    24      $                  (ghost flow from: 511a)
    dec h                      ;5f03    25      %                  (ghost flow from: 511b)
    jr nz,l5f01h               ;5f04    20 fb     .                (ghost flow from: 511c)
l5f06h:
    call 0028eh                ;5f06    cd 8e 02    . . .          (ghost flow from: 511d 5127)
    jr nz,l5f06h               ;5f09    20 fb     .                (ghost flow from: 02b2)
    call 0031eh                ;5f0b    cd 1e 03    . . .          (ghost flow from: 5122)
    jr nc,l5f06h               ;5f0e    30 f6   0 .                (ghost flow from: 0324 0332)
    ld e,a                     ;5f10    5f      _                  (ghost flow from: 5127)
    inc b                      ;5f11    04      .                  (ghost flow from: 5129)
    jr z,l5f16h                ;5f12    28 02   ( .                (ghost flow from: 512a)
    add a,080h                 ;5f14    c6 80   . . 
l5f16h:
    ld hl,0502bh               ;5f16    21 2b 50    ! + P          (ghost flow from: 512b)
    push hl                    ;5f19    e5      .                  (ghost flow from: 512f)
    cp 049h                    ;5f1a    fe 49   . I                (ghost flow from: 5132)
    ld hl,050eeh               ;5f1c    21 ee 50    ! . P          (ghost flow from: 5133)
l5f1fh:
    ld de,00107h               ;5f1f    11 07 01    . . .          (ghost flow from: 5135)
    jr nz,l5f2eh               ;5f22    20 0a     .                (ghost flow from: 5138)
l5f24h:
    ld a,e                     ;5f24    7b      { 
    cpl                        ;5f25    2f      / 
    and (hl)                   ;5f26    a6      . 
    ...
    ret                        ;5f2d    c9      . 
l5f2eh:
    cp 050h                    ;5f2e    fe 50   . P                (ghost flow from: 513b)
l5f30h:
    ld de,00838h               ;5f30    11 38 08    . 8 .          (ghost flow from: 5147)
    jr z,l5f24h                ;5f33    28 ef   ( .                (ghost flow from: 5149)
    cp 042h                    ;5f35    fe 42   . B                (ghost flow from: 514c)
l5f37h:
    ld de,04040h               ;5f37    11 40 40    . @ @          (ghost flow from: 514e)
    jr z,l5f24h                ;5f3a    28 e8   ( .                (ghost flow from: 5150)
    ld hl,050f3h               ;5f3c    21 f3 50    ! . P          (ghost flow from: 5153)
    cp 0c9h                    ;5f3f    fe c9   . .                (ghost flow from: 5155)
    jr z,l5f1fh                ;5f41    28 dc   ( .                (ghost flow from: 5158)
    cp 0d0h                    ;5f43    fe d0   . .                (ghost flow from: 515a)
    jr z,l5f30h                ;5f45    28 e9   ( .                (ghost flow from: 515c)
    cp 0c2h                    ;5f47    fe c2   . .                (ghost flow from: 515e)
    jr z,l5f37h                ;5f49    28 ec   ( .                (ghost flow from: 5160)
    cp 044h                    ;5f4b    fe 44   . D                (ghost flow from: 5162)
    jr nz,l5f57h               ;5f4d    20 08     .                (ghost flow from: 5164)
...
l5f54h:
    ...
    ret                        ;5f56    c9      . 
l5f57h:
    cp 04dh                    ;5f57    fe 4d   . M                (ghost flow from: 5166)
    ld hl,050b7h               ;5f59    21 b7 50    ! . P          (ghost flow from: 5170)
    jr z,l5f54h                ;5f5c    28 f6   ( .                (ghost flow from: 5172)
    cp 058h                    ;5f5e    fe 58   . X                (ghost flow from: 5175)
    ld hl,05102h               ;5f60    21 02 51    ! . Q          (ghost flow from: 5177)
    ld de,0013fh               ;5f63    11 3f 01    . ? .          (ghost flow from: 5179)
    jr z,l5f24h                ;5f66    28 bc   ( .                (ghost flow from: 517c)
    ld d,0ffh                  ;5f68    16 ff   . .                (ghost flow from: 517f)
    cp 0d8h                    ;5f6a    fe d8   . .                (ghost flow from: 5181)
    jr z,l5f24h                ;5f6c    28 b6   ( .                (ghost flow from: 5183)
    cp 043h                    ;5f6e    fe 43   . C                (ghost flow from: 5185)
    jr nz,l5f93h               ;5f70    20 21     !                (ghost flow from: 5187)
...
l5f7ah:
    ...
    and 0dfh                   ;5f91    e6 df   . . 
l5f93h:
    ld hl,050a6h               ;5f93    21 a6 50    ! . P          (ghost flow from: 5189)
    cp 0b0h                    ;5f96    fe b0   . .                (ghost flow from: 51ac)
    jr nz,l5fa9h               ;5f98    20 0f     .                (ghost flow from: 51af)
    dec hl                     ;5f9a    2b      + 
    ld a,(hl)                  ;5f9b    7e      ~ 
    cp 03ah                    ;5f9c    fe 3a   . : 
    ret z                      ;5f9e    c8      . 
...
l5fa3h:
    ...
    ret                        ;5fa8    c9      . 
l5fa9h:
    cp 030h                    ;5fa9    fe 30   . 0                (ghost flow from: 51b1)
    jr c,l5fbah                ;5fab    38 0d   8 .                (ghost flow from: 51c2)
    cp 03ah                    ;5fad    fe 3a   . : 
...
l5fbah:
    cp 00dh                    ;5fba    fe 0d   . .                (ghost flow from: 51c4)
    ret nz                     ;5fbc    c0      .                  (ghost flow from: 51d3)
    pop af                     ;5fbd    f1      .                  (ghost flow from: 51d5)
    pop hl                     ;5fbe    e1      .                  (ghost flow from: 51d6)
    push hl                    ;5fbf    e5      .                  (ghost flow from: 51d7)
    ld (05227h),sp             ;5fc0    ed 73 27 52     . s ' R    (ghost flow from: 51d8)
    ld sp,052c0h               ;5fc4    31 c0 52    1 . R          (ghost flow from: 51d9)
    call 052afh                ;5fc7    cd af 52    . . R          (ghost flow from: 51dd)
    ld a,(05309h)              ;5fca    3a 09 53    : . S          (ghost flow from: 52b3)
    call 052aeh                ;5fcd    cd ae 52    . . R          (ghost flow from: 51e3)
    ld de,(052fah)             ;5fd0    ed 5b fa 52     . [ . R    (ghost flow from: 52b3)
    ld (hl),e                  ;5fd4    73      s                  (ghost flow from: 51e9)
    inc hl                     ;5fd5    23      #                  (ghost flow from: 51ed)
    ld (hl),d                  ;5fd6    72      r                  (ghost flow from: 51ee)
    dec hl                     ;5fd7    2b      +                  (ghost flow from: 51ef)
    call 052afh                ;5fd8    cd af 52    . . R          (ghost flow from: 51f0)
    ld a,(05102h)              ;5fdb    3a 02 51    : . Q          (ghost flow from: 52b3)
    call 052aeh                ;5fde    cd ae 52    . . R          (ghost flow from: 51f4)
    ld a,(050eeh)              ;5fe1    3a ee 50    : . P          (ghost flow from: 52b3)
    call 052aeh                ;5fe4    cd ae 52    . . R          (ghost flow from: 51fa)
    call 052aeh                ;5fe7    cd ae 52    . . R          (ghost flow from: 52b3)
    call 052aeh                ;5fea    cd ae 52    . . R          (ghost flow from: 52b3)
    xor 001h                   ;5fed    ee 01   . .                (ghost flow from: 52b3)
    call 052aeh                ;5fef    cd ae 52    . . R          (ghost flow from: 5206)
    call 052aeh                ;5ff2    cd ae 52    . . R          (ghost flow from: 52b3)
    call 052dch                ;5ff5    cd dc 52    . . R          (ghost flow from: 52b3)
    call 052aeh                ;5ff8    cd ae 52    . . R          (ghost flow from: 52e5)
    and 007h                   ;5ffb    e6 07   . .                (ghost flow from: 52b3)
    call 052aeh                ;5ffd    cd ae 52    . . R          (ghost flow from: 5214)
sub_6000h:
    call 052aeh                ;6000    cd ae 52    . . R          (ghost flow from: 52b3)
    ld a,(050f3h)              ;6003    3a f3 50    : . P          (ghost flow from: 52b3)
    call 052aeh                ;6006    cd ae 52    . . R          (ghost flow from: 521c)
    call 052aeh                ;6009    cd ae 52    . . R          (ghost flow from: 52b3)
    ld (hl),a                  ;600c    77      w                  (ghost flow from: 52b3)
    ld sp,00000h               ;600d    31 00 00    1 . .          (ghost flow from: 5225)
    ld hl,00000h               ;6010    21 00 00    ! . .          (ghost flow from: 5226)
    ld de,050a1h               ;6013    11 a1 50    . . P          (ghost flow from: 5229)
l6016h:
    ld a,(de)                  ;6016    1a      .                  (ghost flow from: 522c 5241)
    inc de                     ;6017    13      .                  (ghost flow from: 522f)
    cp 05fh                    ;6018    fe 5f   . _                (ghost flow from: 5230)
    jr z,l602ah                ;601a    28 0e   ( .                (ghost flow from: 5231)
    add hl,hl                  ;601c    29      )                  (ghost flow from: 5233)
    push hl                    ;601d    e5      .                  (ghost flow from: 5235)
    add hl,hl                  ;601e    29      )                  (ghost flow from: 5236)
    add hl,hl                  ;601f    29      )                  (ghost flow from: 5237)
    pop bc                     ;6020    c1      .                  (ghost flow from: 5238)
    add hl,bc                  ;6021    09      .                  (ghost flow from: 5239)
    sub 030h                   ;6022    d6 30   . 0                (ghost flow from: 523a)
    ld c,a                     ;6024    4f      O                  (ghost flow from: 523b)
    ld b,000h                  ;6025    06 00   . .                (ghost flow from: 523d)
    add hl,bc                  ;6027    09      .                  (ghost flow from: 523e)
    jr l6016h                  ;6028    18 ec   . .                (ghost flow from: 5240)
l602ah:
    pop de                     ;602a    d1      .                  (ghost flow from: 5233)
    ex de,hl                   ;602b    eb      .                  (ghost flow from: 5243)
    ld a,(050b7h)              ;602c    3a b7 50    : . P          (ghost flow from: 5244)
    rrca                       ;602f    0f      .                  (ghost flow from: 5245)
    push af                    ;6030    f5      .                  (ghost flow from: 5248)
    ld bc,03e80h               ;6031    01 80 3e    . . >          (ghost flow from: 5249)
    jr c,l604eh                ;6034    38 18   8 .                (ghost flow from: 524a)
    push hl                    ;6036    e5      . 
    push de                    ;6037    d5      . 
...
l604eh:
    push de                    ;604e    d5      .                  (ghost flow from: 524d)
    push hl                    ;604f    e5      .                  (ghost flow from: 5267)
    xor a                      ;6050    af      .                  (ghost flow from: 5268)
    sbc hl,de                  ;6051    ed 52   . R                (ghost flow from: 5269)
    pop hl                     ;6053    e1      .                  (ghost flow from: 526a)
    jr c,l605ah                ;6054    38 04   8 .                (ghost flow from: 526c)
    ldir                       ;6056    ed b0   . .                (ghost flow from: 526d 526f)
    jr l6062h                  ;6058    18 08   . .                (ghost flow from: 526f)
l605ah:
    add hl,bc                  ;605a    09      . 
    dec hl                     ;605b    2b      + 
    ex de,hl                   ;605c    eb      . 
    ...
    lddr                       ;6060    ed b8   . . 
l6062h:
    pop bc                     ;6062    c1      .                  (ghost flow from: 5271)
    ld d,b                     ;6063    50      P                  (ghost flow from: 527b)
    ld e,c                     ;6064    59      Y                  (ghost flow from: 527c)
    pop af                     ;6065    f1      .                  (ghost flow from: 527d)
    push bc                    ;6066    c5      .                  (ghost flow from: 527e)
    push af                    ;6067    f5      .                  (ghost flow from: 527f)
    jr nc,l6077h               ;6068    30 0d   0 .                (ghost flow from: 5280)
    ld hl,05353h               ;606a    21 53 53    ! S S          (ghost flow from: 5281)
    call 0531ch                ;606d    cd 1c 53    . . S          (ghost flow from: 5283)
    ld hl,001c8h               ;6070    21 c8 01    ! . .          (ghost flow from: 531e)
    add hl,de                  ;6073    19      .                  (ghost flow from: 5289)
    ex de,hl                   ;6074    eb      .                  (ghost flow from: 528c)
    jr l607dh                  ;6075    18 06   . .                (ghost flow from: 528d)
l6077h:
    ...
    ld c,l                     ;607c    4d      M 
l607dh:
    ld hl,05547h               ;607d    21 47 55    ! G U          (ghost flow from: 528e)
    call 0531ch                ;6080    cd 1c 53    . . S          (ghost flow from: 5296)
    ld hl,0f136h               ;6083    21 36 f1    ! 6 .          (ghost flow from: 531e)
    add hl,de                  ;6086    19      .                  (ghost flow from: 529c)
    pop af                     ;6087    f1      .                  (ghost flow from: 529f)
    pop bc                     ;6088    c1      .                  (ghost flow from: 52a0)
    jr c,l608eh                ;6089    38 03   8 .                (ghost flow from: 52a1)
    ld (hl),c                  ;608b    71      q 
    inc hl                     ;608c    23      # 
    ld (hl),b                  ;608d    70      p 
l608eh:
    ld hl,00104h               ;608e    21 04 01    ! . .          (ghost flow from: 52a2)
    add hl,de                  ;6091    19      .                  (ghost flow from: 52a7)
    ld sp,hl                   ;6092    f9      .                  (ghost flow from: 52aa)
    push bc                    ;6093    c5      .                  (ghost flow from: 52ab)
    ret                        ;6094    c9      .                  (ghost flow from: 52ac)
    ld (hl),a                  ;6095    77      w                  (ghost flow from: 51e6 51f7 51fd 5200 5203 5208 520b 5211 5216 5219 521f 5222)
    pop bc                     ;6096    c1      .                  (ghost flow from: 51e0 51f1 52ae)
    pop de                     ;6097    d1      .                  (ghost flow from: 52af)
    add hl,de                  ;6098    19      .                  (ghost flow from: 52b0)
    push bc                    ;6099    c5      .                  (ghost flow from: 52b1)
l609ah:
    ret                        ;609a    c9      .                  (ghost flow from: 52b2)
    ld a,02fh                  ;609b    3e 2f   > /                (ghost flow from: 5012 5018 501e 5023 5028)
l609dh:
    inc a                      ;609d    3c      <                  (ghost flow from: 52b4 52ba)
    and a                      ;609e    a7      .                  (ghost flow from: 52b6)
    sbc hl,de                  ;609f    ed 52   . R                (ghost flow from: 52b7)
    jr nc,l609dh               ;60a1    30 fa   0 .                (ghost flow from: 52b8)
    add hl,de                  ;60a3    19      .                  (ghost flow from: 52ba)
    ld (bc),a                  ;60a4    02      .                  (ghost flow from: 52bc)
    inc bc                     ;60a5    03      .                  (ghost flow from: 52bd)
    ret                        ;60a6    c9      .                  (ghost flow from: 52be)
    ...
    ld e,l                     ;60c1    5d      ] 
    ret m                      ;60c2    f8      . 
    and 0f8h                   ;60c3    e6 f8   . .                (ghost flow from: 50f9 520e)
    ld b,a                     ;60c5    47      G                  (ghost flow from: 52dc)
l60c6h:
    rrca                       ;60c6    0f      .                  (ghost flow from: 52de)
    rrca                       ;60c7    0f      .                  (ghost flow from: 52df)
    rrca                       ;60c8    0f      .                  (ghost flow from: 52e0)
    and 007h                   ;60c9    e6 07   . .                (ghost flow from: 52e1)
    or b                       ;60cb    b0      .                  (ghost flow from: 52e2)
    ret                        ;60cc    c9      .                  (ghost flow from: 52e4)
    ld (05303h),hl             ;60cd    22 03 53    " . S          (ghost flow from: 502e 5050 5068 508a 50ab 50be)
    pop hl                     ;60d0    e1      .                  (ghost flow from: 52e6)
l60d1h:
    ld a,(hl)                  ;60d1    7e      ~                  (ghost flow from: 52e9 5319)
    and 07fh                   ;60d2    e6 7f   .                 (ghost flow from: 52ea)
    exx                        ;60d4    d9      .                  (ghost flow from: 52eb)
    cp 041h                    ;60d5    fe 41   . A                (ghost flow from: 52ed)
    jr c,l60e3h                ;60d7    38 0a   8 .                (ghost flow from: 52ee)
    cp 05bh                    ;60d9    fe 5b   . [                (ghost flow from: 52f0)
    jr c,l60e1h                ;60db    38 04   8 .                (ghost flow from: 52f2)
    cp 061h                    ;60dd    fe 61   . a                (ghost flow from: 52f4)
    jr c,l60e3h                ;60df    38 02   8 .                (ghost flow from: 52f6)
l60e1h:
    and 0ffh                   ;60e1    e6 ff   . .                (ghost flow from: 52f4 52f8)
l60e3h:
    add a,a                    ;60e3    87      .                  (ghost flow from: 52f0 52f8 52fa)
    ld h,00fh                  ;60e4    26 0f   & .                (ghost flow from: 52fc)
    ld l,a                     ;60e6    6f      o                  (ghost flow from: 52fd)
    add hl,hl                  ;60e7    29      )                  (ghost flow from: 52ff)
    add hl,hl                  ;60e8    29      )                  (ghost flow from: 5300)
    ld de,04000h               ;60e9    11 00 40    . . @          (ghost flow from: 5301)
    push de                    ;60ec    d5      .                  (ghost flow from: 5302)
    ld b,008h                  ;60ed    06 08   . .                (ghost flow from: 5305)
l60efh:
    ld a,(hl)                  ;60ef    7e      ~                  (ghost flow from: 5306 530e)
    nop                        ;60f0    00      .                  (ghost flow from: 5308)
    or (hl)                    ;60f1    b6      .                  (ghost flow from: 5309)
    ld (de),a                  ;60f2    12      .                  (ghost flow from: 530a)
    inc hl                     ;60f3    23      #                  (ghost flow from: 530b)
    inc d                      ;60f4    14      .                  (ghost flow from: 530c)
    djnz l60efh                ;60f5    10 f8   . .                (ghost flow from: 530d)
    pop hl                     ;60f7    e1      .                  (ghost flow from: 530e)
    inc l                      ;60f8    2c      ,                  (ghost flow from: 5310)
    ld (05303h),hl             ;60f9    22 03 53    " . S          (ghost flow from: 5311)
    exx                        ;60fc    d9      .                  (ghost flow from: 5312)
    ld a,(hl)                  ;60fd    7e      ~                  (ghost flow from: 5315)
    inc hl                     ;60fe    23      #                  (ghost flow from: 5316)
    rlca                       ;60ff    07      .                  (ghost flow from: 5317)
    jr nc,l60d1h               ;6100    30 cf   0 .                (ghost flow from: 5318)
    jp (hl)                    ;6102    e9      .                  (ghost flow from: 5319)
l6103h:
    ld a,(hl)                  ;6103    7e      ~                  (ghost flow from: 5286 5299 5340)
    or a                       ;6104    b7      .                  (ghost flow from: 531c)
    ret z                      ;6105    c8      .                  (ghost flow from: 531d)
    ld (hl),001h               ;6106    36 01   6 .                (ghost flow from: 531e)
    cp 0c8h                    ;6108    fe c8   . .                (ghost flow from: 531f)
    jr c,l610fh                ;610a    38 03   8 .                (ghost flow from: 5321)
    sub 0c8h                   ;610c    d6 c8   . .                (ghost flow from: 5323)
    inc hl                     ;610e    23      #                  (ghost flow from: 5325)
l610fh:
    push af                    ;610f    f5      .                  (ghost flow from: 5323 5327 533d)
    add a,e                    ;6110    83      .                  (ghost flow from: 5328)
    ld e,a                     ;6111    5f      _                  (ghost flow from: 5329)
    jr nc,l6115h               ;6112    30 01   0 .                (ghost flow from: 532a)
    inc d                      ;6114    14      .                  (ghost flow from: 532b)
l6115h:
    pop af                     ;6115    f1      .                  (ghost flow from: 532b 532d)
    push hl                    ;6116    e5      .                  (ghost flow from: 532e)
    ex de,hl                   ;6117    eb      .                  (ghost flow from: 532f)
    ld e,(hl)                  ;6118    5e      ^                  (ghost flow from: 5330)
    inc hl                     ;6119    23      #                  (ghost flow from: 5331)
    ld d,(hl)                  ;611a    56      V                  (ghost flow from: 5332)
    ex de,hl                   ;611b    eb      .                  (ghost flow from: 5333)
    add hl,bc                  ;611c    09      .                  (ghost flow from: 5334)
    ex de,hl                   ;611d    eb      .                  (ghost flow from: 5335)
    ld (hl),d                  ;611e    72      r                  (ghost flow from: 5336)
    dec hl                     ;611f    2b      +                  (ghost flow from: 5337)
    ld (hl),e                  ;6120    73      s                  (ghost flow from: 5338)
    ex de,hl                   ;6121    eb      .                  (ghost flow from: 5339)
    pop hl                     ;6122    e1      .                  (ghost flow from: 533a)
    dec (hl)                   ;6123    35      5                  (ghost flow from: 533b)
    jr nz,l610fh               ;6124    20 e9     .                (ghost flow from: 533c)
    inc hl                     ;6126    23      #                  (ghost flow from: 533d)
    jr l6103h                  ;6127    18 da   . .                (ghost flow from: 533f)
    ld c,014h                  ;6129    0e 14   . .                (ghost flow from: 5003 5008)
l612bh:
    ld b,008h                  ;612b    06 08   . .                (ghost flow from: 5342 5350)
    push de                    ;612d    d5      .                  (ghost flow from: 5344)
l612eh:
    ld a,(hl)                  ;612e    7e      ~                  (ghost flow from: 5346 534b)
    ld (de),a                  ;612f    12      .                  (ghost flow from: 5347)
    inc hl                     ;6130    23      #                  (ghost flow from: 5348)
    inc d                      ;6131    14      .                  (ghost flow from: 5349)
    djnz l612eh                ;6132    10 fa   . .                (ghost flow from: 534a)
    pop de                     ;6134    d1      .                  (ghost flow from: 534b)
    inc e                      ;6135    1c      .                  (ghost flow from: 534d)
    dec c                      ;6136    0d      .                  (ghost flow from: 534e)
    jr nz,l612bh               ;6137    20 f2     .                (ghost flow from: 534f)
    ret                        ;6139    c9      .                  (ghost flow from: 5350)
...
l65b3h:
    ...
    and b                      ;6705    a0      . 
    ld c,b                     ;6706    48      H 
    call nz,l8201h+2           ;6707    c4 03 82    . . . 
    nop                        ;670a    00      . 
    nop                        ;670b    00      . 
    ld (de),a                  ;670c    12      . 
    ld d,b                     ;670d    50      P 
    and e                      ;670e    a3      . 
    inc bc                     ;670f    03      . 
    ld bc,00000h               ;6710    01 00 00    . . . 
    nop                        ;6713    00      . 
    ld d,b                     ;6714    50      P 
    pop bc                     ;6715    c1      . 
    or d                       ;6716    b2      . 
    ld bc,011f0h               ;6717    01 f0 11    . . . 
    jr nz,l676ch               ;671a    20 50     P 
    jp nz,00192h               ;671c    c2 92 01    . . . 
    jp pe,04011h               ;671f    ea 11 40    . . @ 
    ld d,b                     ;6722    50      P 
    jp 00192h                  ;6723    c3 92 01    . . . 
    jp (hl)                    ;6726    e9      . 
    ld de,05060h               ;6727    11 60 50    . ` P 
    call nz,00192h             ;672a    c4 92 01    . . . 
    call pe,sub_8011h          ;672d    ec 11 80    . . . 
    ld d,b                     ;6730    50      P 
    push bc                    ;6731    c5      . 
    sub d                      ;6732    92      . 
    ld bc,011ebh               ;6733    01 eb 11    . . . 
    and b                      ;6736    a0      . 
    ld d,b                     ;6737    50      P 
    ret z                      ;6738    c8      . 
    sub d                      ;6739    92      . 
    ld bc,011eeh               ;673a    01 ee 11    . . . 
    ret nz                     ;673d    c0      . 
    ld d,b                     ;673e    50      P 
    call z,00192h              ;673f    cc 92 01    . . . 
    defb 0edh                  ;next byte illegal after ed                                     ;6742    ed      . 
    ld de,04880h               ;6743    11 80 48    . . H 
    ret                        ;6746    c9      . 
    add a,d                    ;6747    82      . 
    nop                        ;6748    00      . 
    call c,0c811h              ;6749    dc 11 c8    . . . 
    ld d,b                     ;674c    50      P 
    jp nc,00182h               ;674d    d2 82 01    . . . 
    in a,(011h)                ;6750    db 11   . . 
    nop                        ;6752    00      . 
    ld c,b                     ;6753    48      H 
    add hl,de                  ;6754    19      . 
    add a,d                    ;6755    82      . 
    nop                        ;6756    00      . 
    ret pe                     ;6757    e8      . 
    ld de,04820h               ;6758    11 20 48    .   H 
    dec e                      ;675b    1d      . 
    add a,d                    ;675c    82      . 
    nop                        ;675d    00      . 
    rst 20h                    ;675e    e7      . 
    ld de,04840h               ;675f    11 40 48    . @ H 
    ld a,(de)                  ;6762    1a      . 
    add a,d                    ;6763    82      . 
    nop                        ;6764    00      . 
    and 011h                   ;6765    e6 11   . . 
    ld h,b                     ;6767    60      ` 
    ld c,b                     ;6768    48      H 
    ld e,082h                  ;6769    1e 82   . . 
    nop                        ;676b    00      . 
l676ch:
    push hl                    ;676c    e5      . 
    ld de,050ceh               ;676d    11 ce 50    . . P 
    add a,000h                 ;6770    c6 00   . . 
    ld bc,011efh               ;6772    01 ef 11    . . . 
    ret po                     ;6775    e0      . 
    ld b,b                     ;6776    40      @ 
    dec d                      ;6777    15      . 
    adc a,d                    ;6778    8a      . 
    nop                        ;6779    00      . 
    rst 28h                    ;677a    ef      . 
    ld de,05049h               ;677b    11 49 50    . I P 
    ld d,08ah                  ;677e    16 8a   . . 
    ld bc,011e9h               ;6780    01 e9 11    . . . 
    ld l,c                     ;6783    69      i 
    ld d,b                     ;6784    50      P 
    rla                        ;6785    17      . 
    adc a,d                    ;6786    8a      . 
l6787h:
    ld bc,011ebh               ;6787    01 eb 11    . . . 
    adc a,c                    ;678a    89      . 
    ld d,b                     ;678b    50      P 
    jr $-116                   ;678c    18 8a   . . 
    ld bc,011edh               ;678e    01 ed 11    . . . 
    ld d,d                     ;6791    52      R 
    ld d,b                     ;6792    50      P 
    inc hl                     ;6793    23      # 
    adc a,d                    ;6794    8a      . 
    ld bc,011f1h               ;6795    01 f1 11    . . . 
    ld (hl),d                  ;6798    72      r 
    ld d,b                     ;6799    50      P 
    dec de                     ;679a    1b      . 
    adc a,d                    ;679b    8a      . 
    ld bc,011e7h               ;679c    01 e7 11    . . . 
    sub d                      ;679f    92      . 
    ld d,b                     ;67a0    50      P 
l67a1h:
    inc e                      ;67a1    1c      . 
    adc a,d                    ;67a2    8a      . 
    ld bc,011e5h               ;67a3    01 e5 11    . . . 
    exx                        ;67a6    d9      . 
    ld d,b                     ;67a7    50      P 
    call nc,0018ah             ;67a8    d4 8a 01    . . . 
    di                         ;67ab    f3      . 
    ld de,048a0h               ;67ac    11 a0 48    . . H 
    ret c                      ;67af    d8      . 
    add a,c                    ;67b0    81      . 
    ret po                     ;67b1    e0      . 
    push af                    ;67b2    f5      . 
    ld de,048b1h               ;67b3    11 b1 48    . . H 
    exx                        ;67b6    d9      . 
    add a,c                    ;67b7    81      . 
    ret po                     ;67b8    e0      . 
    rst 30h                    ;67b9    f7      . 
    ld de,04808h               ;67ba    11 08 48    . . H 
    ld h,081h                  ;67bd    26 81   & . 
    ret po                     ;67bf    e0      . 
    jp (hl)                    ;67c0    e9      . 
    ld de,04828h               ;67c1    11 28 48    . ( H 
    daa                        ;67c4    27      ' 
    add a,c                    ;67c5    81      . 
    ret po                     ;67c6    e0      . 
    ex de,hl                   ;67c7    eb      . 
    ld de,04848h               ;67c8    11 48 48    . H H 
    jr z,$-125                 ;67cb    28 81   ( . 
    ret po                     ;67cd    e0      . 
    defb 0edh                  ;next byte illegal after ed                                     ;67ce    ed      . 
    ld de,05016h               ;67cf    11 16 50    . . P 
    dec hl                     ;67d2    2b      + 
    adc a,l                    ;67d3    8d      . 
    push hl                    ;67d4    e5      . 
    pop af                     ;67d5    f1      . 
    ld de,04868h               ;67d6    11 68 48    . h H 
    add hl,hl                  ;67d9    29      ) 
    add a,c                    ;67da    81      . 
    ret po                     ;67db    e0      . 
    rst 20h                    ;67dc    e7      . 
    ld de,04888h               ;67dd    11 88 48    . . H 
    ld hl,(0e081h)             ;67e0    2a 81 e0    * . . 
    push hl                    ;67e3    e5      . 
    ld de,002d4h               ;67e4    11 d4 02    . . . 
    ret nz                     ;67e7    c0      . 
    ld e,l                     ;67e8    5d      ] 
    jr c,l6787h                ;67e9    38 9c   8 . 
    nop                        ;67eb    00      . 
    nop                        ;67ec    00      . 
    nop                        ;67ed    00      . 
    nop                        ;67ee    00      . 
...
l75d5h:
    ...
    ld a,h                     ;75da    7c      | 
    sub 007h                   ;75db    d6 07   . .                (flow from: 88bc)
    jr l75d5h                  ;75dd    18 f6   . .                (flow from: 75db)
l75dfh:
    ld e,008h                  ;75df    1e 08   . . 
l75e1h:
    ld (hl),a                  ;75e1    77      w 
    inc h                      ;75e2    24      $                  (flow from: 75e0)
    dec e                      ;75e3    1d      . 
    jr nz,l75e1h               ;75e4    20 fb     . 
    pop hl                     ;75e6    e1      . 
l75e7h:
    inc hl                     ;75e7    23      #                  (flow from: 75e5)
    djnz l75cch                ;75e8    10 e2   . .                (flow from: 75e7)
    dec c                      ;75ea    0d      . 
...
l7691h:
    ...
    jr l76beh                  ;76b6    18 06   . .                (flow from: 89ec)
    ld ix,00003h               ;76b8    dd 21 03 00     . ! . .    (flow from: 76b6)
    ld b,022h                  ;76bc    06 22   . "                (flow from: 76bb)
l76beh:
    ld (010b9h),hl             ;76be    22 b9 10    " . . 
l76c1h:
    push bc                    ;76c1    c5      .                  (flow from: 76bf 76cb)
    call 00febh                ;76c2    cd eb 0f    . . . 
    ld bc,00007h               ;76c5    01 07 00    . . .          (flow from: 76c4)
    add ix,bc                  ;76c8    dd 09   . .                (flow from: 76c6)
    pop bc                     ;76ca    c1      .                  (flow from: 76c9)
    djnz l76c1h                ;76cb    10 f4   . .                (flow from: 76ca)
    ret                        ;76cd    c9      .                  (flow from: 76cb)
l76ceh:
...
l76feh:
    ...
    call 011afh                ;770a    cd af 11    . . .          (flow from: 77fd)
l770dh:
    ld a,03ah                  ;770d    3e 3a   > :                (flow from: 7bc6)
    call 029a9h                ;770f    cd a9 29    . . ) 
    pop de                     ;7712    d1      . 
    ld a,(ix+004h)             ;7713    dd 7e 04    . ~ . 
    and 01fh                   ;7716    e6 1f   . .                (flow from: 7e06)
    ld hl,(029c8h)             ;7718    2a c8 29    * . ) 
l771bh:
    ld bc,0112ch               ;771b    01 2c 11    . , . 
    or a                       ;771e    b7      . 
    ret z                      ;771f    c8      .                  (flow from: 771c 776d)
    push af                    ;7720    f5      . 
    push hl                    ;7721    e5      . 
    ld a,(de)                  ;7722    1a      .                  (flow from: 771f)
    ld l,a                     ;7723    6f      o                  (flow from: 7722)
    inc de                     ;7724    13      .                  (flow from: 7723)
    ld a,(de)                  ;7725    1a      . 
    ld h,a                     ;7726    67      g                  (flow from: 7724)
    ld a,(ix+003h)             ;7727    dd 7e 03    . ~ . 
    bit 3,a                    ;772a    cb 5f   . _ 
    jr z,l772fh                ;772c    28 01   ( .                (flow from: 7729)
    inc de                     ;772e    13      . 
l772fh:
    push de                    ;772f    d5      . 
    ld e,a                     ;7730    5f      _                  (flow from: 772d)
    and 003h                   ;7731    e6 03   . .                (flow from: 7730)
    jp z,010dch                ;7733    ca dc 10    . . .          (flow from: 7731)
    cp 003h                    ;7736    fe 03   . . 
    jr nc,l7777h               ;7738    30 3d   0 = 
    ld d,004h                  ;773a    16 04   . .                (flow from: 7737)
l773ch:
    bit 3,(ix+003h)            ;773c    dd cb 03 5e     . . . ^ 
    push bc                    ;7740    c5      .                  (flow from: 8189 81b6 81dd 822d 8231)
...
l7744h:
    ...
    ld a,(bc)                  ;774a    0a      . 
    ld b,000h                  ;774b    06 00   . .                (flow from: 7748)
    ld c,a                     ;774d    4f      O                  (flow from: 774b)
    add ix,bc                  ;774e    dd 09   . . 
    rl e                       ;7750    cb 13   . . 
    push de                    ;7752    d5      .                  (flow from: 774f)
    push hl                    ;7753    e5      . 
    call c,01134h              ;7754    dc 34 11    . 4 . 
    pop hl                     ;7757    e1      . 
    pop de                     ;7758    d1      .                  (flow from: 7755 7758)
    pop ix                     ;7759    dd e1   . . 
    pop bc                     ;775b    c1      . 
    inc bc                     ;775c    03      . 
    inc bc                     ;775d    03      .                  (flow from: 82e5)
    dec d                      ;775e    15      . 
    jr nz,l773ch               ;775f    20 db     . 
    pop de                     ;7761    d1      . 
    pop hl                     ;7762    e1      . 
    bit 2,(ix+003h)            ;7763    dd cb 03 56     . . . V    (flow from: 8268)
    jr z,l7773h                ;7767    28 0a   ( .                (flow from: 7763)
...
l7773h:
    pop af                     ;7773    f1      . 
    dec a                      ;7774    3d      =                  (flow from: 7733)
    jr l771bh                  ;7775    18 a4   . . 
l7777h:
    pop de                     ;7777    d1      .                  (flow from: 7774)
    pop hl                     ;7778    e1      . 
    pop af                     ;7779    f1      . 
    call 010cah                ;777a    cd ca 10    . . .          (flow from: 7777)
    rlca                       ;777d    07      . 
    rlca                       ;777e    07      . 
    rlca                       ;777f    07      . 
    ld l,a                     ;7780    6f      o 
...
l77b1h:
    ...
    ld (hl),e                  ;77c8    73      s 
    ld a,d                     ;77c9    7a      z 
    dec l                      ;77ca    2d      -                  (flow from: 773a 7745)
    ld l,b                     ;77cb    68      h 
    dec l                      ;77cc    2d      - 
    ld (hl),b                  ;77cd    70      p                  (flow from: 77ca)
    ld l,(hl)                  ;77ce    6e      n 
    ex (sp),hl                 ;77cf    e3      . 
    pop af                     ;77d0    f1      .                  (flow from: 77cd)
    pop af                     ;77d1    f1      .                  (flow from: 89ef)
    ld a,l                     ;77d2    7d      } 
    ex af,af'                  ;77d3    08      . 
    ld c,l                     ;77d4    4d      M                  (flow from: 7c0c)
    ld b,004h                  ;77d5    06 04   . . 
    bit 5,e                    ;77d7    cb 6b   . k                (flow from: 7c29)
    call 010cah                ;77d9    cd ca 10    . . .          (flow from: 77d7)
    push hl                    ;77dc    e5      .                  (flow from: 77d9)
    ld hl,01120h               ;77dd    21 20 11    !   .          (flow from: 77dc)
    jr z,l77f8h                ;77e0    28 16   ( . 
    ld de,010d4h               ;77e2    11 d4 10    . . .          (flow from: 77e1)
    call 011b5h                ;77e5    cd b5 11    . . .          (flow from: 77e2)
    pop hl                     ;77e8    e1      .                  (flow from: 77e6)
    call 011d4h                ;77e9    cd d4 11    . . .          (flow from: 77e8)
    ld (029c8h),hl             ;77ec    22 c8 29    " . )          (flow from: 77e9)
    ex af,af'                  ;77ef    08      . 
    call 01176h                ;77f0    cd 76 11    . v .          (flow from: 77ec)
    pop hl                     ;77f3    e1      . 
    ret                        ;77f4    c9      .                  (flow from: 77f1)
l77f5h:
    call 029a5h                ;77f5    cd a5 29    . . )          (flow from: 77f4)
l77f8h:
    ld a,(hl)                  ;77f8    7e      ~ 
    inc hl                     ;77f9    23      #                  (flow from: 77f6)
    and c                      ;77fa    a1      . 
    jr nz,l7801h               ;77fb    20 04     .                (flow from: 77f9)
    inc hl                     ;77fd    23      #                  (flow from: 77fc)
sub_77feh:
    ld a,(hl)                  ;77fe    7e      ~ 
    jr l7803h                  ;77ff    18 02   . . 
l7801h:
    ld a,(hl)                  ;7801    7e      ~ 
    inc hl                     ;7802    23      #                  (flow from: 7800)
l7803h:
    inc hl                     ;7803    23      # 
    push af                    ;7804    f5      .                  (flow from: 7802)
    rlca                       ;7805    07      . 
...
l7ba8h:
    ...
    jr l7b87h                  ;7bb3    18 d2   . .                (flow from: 7bb2)
l7bb5h:
    inc a                      ;7bb5    3c      <                  (flow from: 7bb4)
    ld (de),a                  ;7bb6    12      . 
    ld (0205fh),a              ;7bb7    32 5f 20    2 _            (flow from: 7bb5)
    dec a                      ;7bba    3d      = 
    inc c                      ;7bbb    0c      . 
l7bbch:
    ld (de),a                  ;7bbc    12      .                  (flow from: 0000)
    dec c                      ;7bbd    0d      .                  (flow from: 7bbc)
    jr nz,l7bbch               ;7bbe    20 fc     . 
    ld hl,014d5h               ;7bc0    21 d5 14    ! . . 
    ld (02079h),hl             ;7bc3    22 79 20    " y   
    jp 0201dh                  ;7bc6    c3 1d 20    . .            (flow from: 7bc5)
    ...
    jr nz,l7bf2h               ;7be0    20 10     .                (flow from: 7bdc)
    xor a                      ;7be2    af      . 
l7be3h:
    ld de,03e66h               ;7be3    11 66 3e    . f >          (flow from: 7be0)
    ld (0151eh),de             ;7be6    ed 53 1e 15     . S . . 
    ld (0152eh),a              ;7bea    32 2e 15    2 . .          (flow from: 7be8)
    call 01e6fh                ;7bed    cd 6f 1e    . o . 
    jr l7bfdh                  ;7bf0    18 0b   . . 
l7bf2h:
    ld b,042h                  ;7bf2    06 42   . B                (flow from: 7bef)
    call 01e73h                ;7bf4    cd 73 1e    . s .          (flow from: 7bf2 7c03)
    jr nz,l7bfdh               ;7bf7    20 04     .                (flow from: 7bf5)
    ld a,001h                  ;7bf9    3e 01   > .                (flow from: 7bf7)
    jr l7be3h                  ;7bfb    18 e6   . .                (flow from: 7bf9)
l7bfdh:
    ld b,03ah                  ;7bfd    06 3a   . : 
    call 01e73h                ;7bff    cd 73 1e    . s .          (flow from: 7bfb)
    ld de,015ceh               ;7c02    11 ce 15    . . .          (flow from: 8749)
    call z,01544h              ;7c05    cc 44 15    . D .          (flow from: 7c03)
    call 02c32h                ;7c08    cd 32 2c    . 2 ,          (flow from: 7c05)
...
l7c11h:
    ...
    call 02c6fh                ;7c1a    cd 6f 2c    . o ,          (flow from: 7c15)
    ret nc                     ;7c1d    d0      . 
    push hl                    ;7c1e    e5      . 
    pop ix                     ;7c1f    dd e1   . .                (flow from: 7c1c)
    ld a,000h                  ;7c21    3e 00   > . 
    or a                       ;7c23    b7      .                  (flow from: 7c22)
    jr z,l7c2bh                ;7c24    28 05   ( .                (flow from: 7c23)
    call 02335h                ;7c26    cd 35 23    . 5 #          (flow from: 7c25)
    jr c,l7c36h                ;7c29    38 0b   8 .                (flow from: 7c27)
l7c2bh:
    call 02375h                ;7c2b    cd 75 23    . u # 
    call 015ach                ;7c2e    cd ac 15    . . . 
    ld hl,(0151eh)             ;7c31    2a 1e 15    * . . 
    jr c,l7c9ch                ;7c34    38 66   8 f                (flow from: 7c33)
l7c36h:
...
l7c3bh:
    ...
    ld (de),a                  ;7c44    12      . 
    inc de                     ;7c45    13      . 
    inc b                      ;7c46    04      .                  (flow from: 7e06)
    jr l7c3bh                  ;7c47    18 f2   . . 
l7c49h:
    pop hl                     ;7c49    e1      . 
    dec hl                     ;7c4a    2b      +                  (flow from: 7c46)
    ld (hl),b                  ;7c4b    70      p 
    ret                        ;7c4c    c9      .                  (flow from: 7c4a)
    call 01e6ah                ;7c4d    cd 6a 1e    . j .          (flow from: 7c4c)
...
l7c55h:
    ...
    ei                         ;7c6b    fb      . 
    ld hl,00010h               ;7c6c    21 10 00    ! . .          (flow from: 7d77)
    call 024e3h                ;7c6f    cd e3 24    . . $          (flow from: 8244)
    ld a,00dh                  ;7c72    3e 0d   > .                (flow from: 8a2e)
    rst 10h                    ;7c74    d7      . 
    xor a                      ;7c75    af      .                  (flow from: 7d3e)
    ret                        ;7c76    c9      . 
    call 02327h                ;7c77    cd 27 23    . ' # 
    ld bc,00001h               ;7c7a    01 01 00    . . . 
l7c7dh:
    push hl                    ;7c7d    e5      . 
    and a                      ;7c7e    a7      .                  (flow from: 7c7c)
    sbc hl,de                  ;7c7f    ed 52   . R                (flow from: 7c7e)
    pop hl                     ;7c81    e1      . 
    jr nc,l7c8ah               ;7c82    30 06   0 .                (flow from: 7c7f)
    inc bc                     ;7c84    03      . 
...
l7ca3h:
    push de                    ;7ca3    d5      .                  (flow from: 7ccc)
    call 015b9h                ;7ca4    cd b9 15    . . . 
    pop de                     ;7ca7    d1      .                  (flow from: 7ca6)
    ret c                      ;7ca8    d8      . 
    inc de                     ;7ca9    13      . 
    jr nz,l7ca3h               ;7caa    20 f7     .                (flow from: 877e)
    ret                        ;7cac    c9      .                  (flow from: 7caa)
    ld hl,015cdh               ;7cad    21 cd 15    ! . .          (flow from: 7cac)
    ld b,(hl)                  ;7cb0    46      F 
l7cb1h:
    inc hl                     ;7cb1    23      # 
l7cb2h:
    ...
    ex de,hl                   ;7cc7    eb      . 
    ex af,af'                  ;7cc8    08      . 
    add a,d                    ;7cc9    82      .                  (flow from: 5dc0)
    ld l,c                     ;7cca    69      i                  (flow from: 7cc9)
    jp nz,00318h               ;7ccb    c2 18 03    . . . 
    nop                        ;7cce    00      . 
    ld a,b                     ;7ccf    78      x                  (flow from: 777a 7caf 7e38)
    ld l,a                     ;7cd0    6f      o 
    ld (hl),e                  ;7cd1    73      s 
    ld h,l                     ;7cd2    65      e                  (flow from: 7ccf)
    ld h,(hl)                  ;7cd3    66      f 
    ld (hl),e                  ;7cd4    73      s 
    jr nc,$-59                 ;7cd5    30 c3   0 .                (flow from: 7cd2)
    push de                    ;7cd7    d5      . 
    ex af,af'                  ;7cd8    08      .                  (flow from: 82e5)
    add a,d                    ;7cd9    82      . 
    ld l,d                     ;7cda    6a      j                  (flow from: 7cd8)
    jp nz,00000h               ;7cdb    c2 00 00    . . . 
    nop                        ;7cde    00      . 
    ld (bc),a                  ;7cdf    02      . 
    add a,b                    ;7ce0    80      .                  (flow from: 7cdd)
    ret c                      ;7ce1    d8      . 
    jp nz,000c5h               ;7ce2    c2 c5 00    . . . 
    push bc                    ;7ce5    c5      . 
    nop                        ;7ce6    00      .                  (flow from: 82e5)
    ex de,hl                   ;7ce7    eb      . 
    nop                        ;7ce8    00      . 
    ld hl,(0cd02h)             ;7ce9    2a 02 cd    * . .          (flow from: 7ce6)
    ld hl,0c373h               ;7cec    21 73 c3    ! s .          (flow from: 7ceb)
    xor (hl)                   ;7cef    ae      . 
    ld l,c                     ;7cf0    69      i 
    ld (060a8h),a              ;7cf1    32 a8 60    2 . `          (flow from: 7cee)
    ld a,010h                  ;7cf4    3e 10   > .                (flow from: 7d71 828e)
    ld (01612h),a              ;7cf6    32 12 16    2 . . 
    ld a,010h                  ;7cf9    3e 10   > . 
    push af                    ;7cfb    f5      . 
    ex af,af'                  ;7cfc    08      . 
    ld a,b                     ;7cfd    78      x                  (flow from: 86a0 86ad)
    or c                       ;7cfe    b1      .                  (flow from: 7cfd)
    jp z,01f06h                ;7cff    ca 06 1f    . . . 
    ex af,af'                  ;7d02    08      . 
    push bc                    ;7d03    c5      .                  (flow from: 8a1d)
    push hl                    ;7d04    e5      . 
    ld c,000h                  ;7d05    0e 00   . .                (flow from: 7d03)
    call 022b0h                ;7d07    cd b0 22    . . " 
    ld (029c8h),hl             ;7d0a    22 c8 29    " . ) 
    pop hl                     ;7d0d    e1      .                  (flow from: 7d0b)
    push hl                    ;7d0e    e5      . 
    ld de,00000h               ;7d0f    11 00 00    . . .          (flow from: 7d0d)
    and a                      ;7d12    a7      . 
    sbc hl,de                  ;7d13    ed 52   . R 
    ex de,hl                   ;7d15    eb      .                  (flow from: 7d11)
    ld hl,(02a17h)             ;7d16    2a 17 2a    * . * 
    ld c,(hl)                  ;7d19    4e      N                  (flow from: 7d18)
    inc hl                     ;7d1a    23      # 
    ld b,(hl)                  ;7d1b    46      F 
l7d1ch:
    inc hl                     ;7d1c    23      #                  (flow from: 7d19)
    ld a,(hl)                  ;7d1d    7e      ~ 
    inc hl                     ;7d1e    23      # 
    push hl                    ;7d1f    e5      .                  (flow from: 82e5)
    ld h,(hl)                  ;7d20    66      f                  (flow from: 7d1f)
    ld l,a                     ;7d21    6f      o 
    ld a,h                     ;7d22    7c      | 
    and 03fh                   ;7d23    e6 3f   . ?                (flow from: 81b6)
    ld h,a                     ;7d25    67      g                  (flow from: 7d23)
    sbc hl,de                  ;7d26    ed 52   . R 
    pop hl                     ;7d28    e1      .                  (flow from: 7d25)
    jr z,l7d35h                ;7d29    28 0a   ( . 
    dec bc                     ;7d2b    0b      . 
    ld a,b                     ;7d2c    78      x                  (flow from: 7d2a)
    or c                       ;7d2d    b1      . 
...
l7d35h:
    ld c,(hl)                  ;7d35    4e      N 
    pop hl                     ;7d36    e1      .                  (flow from: 8a1d)
    ld e,(hl)                  ;7d37    5e      ^ 
    inc hl                     ;7d38    23      #                  (flow from: 7d2c)
    ld d,(hl)                  ;7d39    56      V 
    inc hl                     ;7d3a    23      #                  (flow from: 7d38)
    push de                    ;7d3b    d5      . 
    ex de,hl                   ;7d3c    eb      .                  (flow from: 7d3a)
...
l7d6ah:
    ...
    or a                       ;7d72    b7      . 
    call nz,0156fh             ;7d73    c4 6f 15    . o .          (flow from: 7d47)
    pop hl                     ;7d76    e1      . 
    pop bc                     ;7d77    c1      .                  (flow from: 7d75 7d7a)
    dec bc                     ;7d78    0b      . 
    pop af                     ;7d79    f1      . 
    add a,008h                 ;7d7a    c6 08   . .                (flow from: 7c82)
    cp 0a9h                    ;7d7c    fe a9   . .                (flow from: 7d7a)
    jp c,01607h                ;7d7e    da 07 16    . . .          (flow from: 7d75)
    ret                        ;7d81    c9      . 
l7d82h:
...
l7da0h:
    ...
    ld c,(hl)                  ;7dab    4e      N 
    inc hl                     ;7dac    23      #                  (flow from: 7d80)
    ld b,(hl)                  ;7dad    46      F 
    inc hl                     ;7dae    23      #                  (flow from: 7dac)
    add hl,bc                  ;7daf    09      . 
    add hl,bc                  ;7db0    09      . 
...
l7dbbh:
    ...
    exx                        ;7dc6    d9      .                  (flow from: 7dae)
    xor a                      ;7dc7    af      . 
    call 01602h                ;7dc8    cd 02 16    . . .          (flow from: 7dc6)
...
l7dd6h:
    ...
    ld b,04ch                  ;7de3    06 4c   . L                (flow from: 7de0)
    call 01e73h                ;7de5    cd 73 1e    . s . 
    ld c,0feh                  ;7de8    0e fe   . . 
    jr z,l7df5h                ;7dea    28 09   ( . 
    ld b,055h                  ;7dec    06 55   . U                (flow from: 85b1)
    call 01e73h                ;7dee    cd 73 1e    . s .          (flow from: 7dec)
...
l7df5h:
    ...
    ld c,(hl)                  ;7dfc    4e      N                  (flow from: 7dfb)
    inc hl                     ;7dfd    23      #                  (flow from: 7dfc)
    ld b,(hl)                  ;7dfe    46      F 
    inc hl                     ;7dff    23      # 
l7e00h:
    ld a,b                     ;7e00    78      x                  (flow from: 7dfd)
    or c                       ;7e01    b1      .                  (flow from: 7e00)
    ret z                      ;7e02    c8      .                  (flow from: 7e01)
    inc hl                     ;7e03    23      #                  (flow from: 7e02)
    set 7,(hl)                 ;7e04    cb fe   . .                (flow from: 7e03)
    inc hl                     ;7e06    23      #                  (flow from: 7e05)
    dec bc                     ;7e07    0b      .                  (flow from: 7df2)
    jr l7e00h                  ;7e08    18 f6   . . 
    ld b,079h                  ;7e0a    06 79   . y                (flow from: 8000)
    call 01e6ch                ;7e0c    cd 6c 1e    . l . 
    ret nz                     ;7e0f    c0      . 
    ld hl,03e68h               ;7e10    21 68 3e    ! h > 
    ld (02328h),hl             ;7e13    22 28 23    " ( # 
    call 013ceh                ;7e16    cd ce 13    . . .          (flow from: 7e15)
...
l7e2dh:
    ...
    push hl                    ;7e2f    e5      .                  (flow from: 7e21 8989)
    ld l,(hl)                  ;7e30    6e      n 
    and 07fh                   ;7e31    e6 7f   .                 (flow from: 7e2f)
    ld h,a                     ;7e33    67      g 
    add hl,hl                  ;7e34    29      )                  (flow from: 7e31)
    ld de,(02a17h)             ;7e35    ed 5b 17 2a     . [ . *    (flow from: 7e34)
    add hl,de                  ;7e39    19      . 
    inc hl                     ;7e3a    23      # 
    set 6,(hl)                 ;7e3b    cb f6   . .                (flow from: 7dd8)
    pop hl                     ;7e3d    e1      . 
    inc hl                     ;7e3e    23      #                  (flow from: 7e3b)
...
l7e44h:
    ...
    pop hl                     ;7e51    e1      .                  (flow from: 7e43)
l7e52h:
    ld a,b                     ;7e52    78      x 
    or c                       ;7e53    b1      .                  (flow from: 7e51)
...
l7e5bh:
    ...
    ld e,(hl)                  ;7e5f    5e      ^                  (flow from: 7e53)
    inc hl                     ;7e60    23      # 
    ld a,(hl)                  ;7e61    7e      ~                  (flow from: 7e5f)
    and 0c0h                   ;7e62    e6 c0   . . 
    ld a,(hl)                  ;7e64    7e      ~                  (flow from: 7e63)
    inc hl                     ;7e65    23      #                  (flow from: 7e64)
    jr nz,l7e52h               ;7e66    20 ea     .                (flow from: 7e65)
    and 03fh                   ;7e68    e6 3f   . ? 
    ld d,a                     ;7e6a    57      W 
    push bc                    ;7e6b    c5      .                  (flow from: 7e69 7e70)
    push hl                    ;7e6c    e5      .                  (flow from: 7e6b)
    ld hl,00000h               ;7e6d    21 00 00    ! . .          (flow from: 7e6c)
    add hl,de                  ;7e70    19      .                  (flow from: 7e6f)
    ld (017d0h),de             ;7e71    ed 53 d0 17     . S . . 
    ld d,h                     ;7e75    54      T 
l7e76h:
    ld e,l                     ;7e76    5d      ]                  (flow from: 7e74)
    inc de                     ;7e77    13      . 
    inc de                     ;7e78    13      .                  (flow from: 7e76)
l7e79h:
    ld a,(de)                  ;7e79    1a      . 
    inc de                     ;7e7a    13      . 
    rlca                       ;7e7b    07      .                  (flow from: 7e78)
    jr nc,l7e79h               ;7e7c    30 fb   0 . 
    call 02b21h                ;7e7e    cd 21 2b    . ! +          (flow from: 7e7d)
    and a                      ;7e81    a7      .                  (flow from: 7e76)
    sbc hl,de                  ;7e82    ed 52   . R 
    ld b,h                     ;7e84    44      D                  (flow from: 7e83)
    ld c,l                     ;7e85    4d      M                  (flow from: 7e84)
...
l7ea8h:
    ...
    inc de                     ;7eaa    13      . 
    dec bc                     ;7eab    0b      . 
    ld a,b                     ;7eac    78      x                  (flow from: 7e85)
    or c                       ;7ead    b1      . 
    jr nz,l7ea8h               ;7eae    20 f8     .                (flow from: 7eac)
    ld hl,(02a17h)             ;7eb0    2a 17 2a    * . * 
    push hl                    ;7eb3    e5      .                  (flow from: 7eb2)
    inc bc                     ;7eb4    03      .                  (flow from: 7eb3)
    call 02c11h                ;7eb5    cd 11 2c    . . ,          (flow from: 7eb4)
    pop ix                     ;7eb8    dd e1   . .                (flow from: 7eb6)
    pop bc                     ;7eba    c1      . 
l7ebbh:
    ld a,d                     ;7ebb    7a      z                  (flow from: 7eb9)
    or e                       ;7ebc    b3      .                  (flow from: 7ebb)
    jr z,l7ed5h                ;7ebd    28 16   ( . 
    push de                    ;7ebf    d5      .                  (flow from: 7ebe)
    call 0181bh                ;7ec0    cd 1b 18    . . . 
    ld de,00000h               ;7ec3    11 00 00    . . .          (flow from: 7ec2)
    and a                      ;7ec6    a7      . 
    sbc hl,de                  ;7ec7    ed 52   . R                (flow from: 7ec5)
...
l7ee9h:
    jr nc,l7f08h               ;7ee9    30 1d   0 .                (flow from: 7ee7)
    push hl                    ;7eeb    e5      .                  (flow from: 7ee9)
    ld l,(hl)                  ;7eec    6e      n 
    ld h,a                     ;7eed    67      g                  (flow from: 7eeb)
    push hl                    ;7eee    e5      . 
    res 7,h                    ;7eef    cb bc   . . 
    or a                       ;7ef1    b7      . 
    ...
    pop hl                     ;7ef7    e1      . 
l7ef8h:
    inc hl                     ;7ef8    23      #                  (flow from: 7ef6)
    call 01838h                ;7ef9    cd 38 18    . 8 . 
    jr l7ee9h                  ;7efc    18 eb   . . 
l7efeh:
    dec hl                     ;7efe    2b      +                  (flow from: 85ba)
    ld b,h                     ;7eff    44      D 
    ld c,l                     ;7f00    4d      M 
    pop hl                     ;7f01    e1      .                  (flow from: 7efe)
    ld (hl),c                  ;7f02    71      q 
    dec hl                     ;7f03    2b      +                  (flow from: 7f01)
    ld (hl),b                  ;7f04    70      p 
    inc hl                     ;7f05    23      # 
    jr l7ef8h                  ;7f06    18 f0   . .                (flow from: 8585 8590)
l7f08h:
    ld hl,00000h               ;7f08    21 00 00    ! . .          (flow from: 7f06)
    pop bc                     ;7f0b    c1      . 
    jp 0175eh                  ;7f0c    c3 5e 17    . ^ .          (flow from: 7f06 7f09)
    inc ix                     ;7f0f    dd 23   . #                (flow from: 7f0c)
    ld l,(ix+001h)             ;7f11    dd 6e 01    . n .          (flow from: 7f0f)
    inc ix                     ;7f14    dd 23   . #                (flow from: 8585)
    ld a,(ix+001h)             ;7f16    dd 7e 01    . ~ . 
    and 03fh                   ;7f19    e6 3f   . ? 
    ld h,a                     ;7f1b    67      g                  (flow from: 7f1a)
    ret                        ;7f1c    c9      .                  (flow from: 7f1b)
l7f1dh:
    call 02c6fh                ;7f1d    cd 6f 2c    . o , 
    ret nc                     ;7f20    d0      . 
    inc hl                     ;7f21    23      # 
    ld a,(hl)                  ;7f22    7e      ~                  (flow from: 8522)
    and 00fh                   ;7f23    e6 0f   . . 
    inc hl                     ;7f25    23      #                  (flow from: 7f22)
    jr z,l7f1dh                ;7f26    28 f5   ( . 
    and 008h                   ;7f28    e6 08   . .                (flow from: 856e)
    jr nz,l7f37h               ;7f2a    20 0b     . 
l7f2ch:
    ld a,(hl)                  ;7f2c    7e      ~                  (flow from: 8538)
    inc hl                     ;7f2d    23      # 
    cp 0c0h                    ;7f2e    fe c0   . . 
    jr nc,l7f1dh               ;7f30    30 eb   0 . 
    cp 080h                    ;7f32    fe 80   . .                (flow from: 7f2f)
    jr c,l7f2ch                ;7f34    38 f6   8 .                (flow from: 7f32)
    dec hl                     ;7f36    2b      +                  (flow from: 7f34)
l7f37h:
    ld a,(hl)                  ;7f37    7e      ~ 
    inc hl                     ;7f38    23      #                  (flow from: 7f36)
    scf                        ;7f39    37      7 
    ret                        ;7f3a    c9      . 
    ld ix,050e0h               ;7f3b    dd 21 e0 50     . ! . P    (flow from: 7f34 7f38)
    ld (ix+000h),003h          ;7f3f    dd 36 00 03     . 6 . .    (flow from: 7f3e)
    call 01e6ah                ;7f43    cd 6a 1e    . j .          (flow from: 7f42)
    push af                    ;7f46    f5      . 
    ld b,03ah                  ;7f47    06 3a   . :                (flow from: 7f45)
    cp b                       ;7f49    b8      . 
    jr z,l7f4fh                ;7f4a    28 03   ( .                (flow from: 7f47)
    call 01e6fh                ;7f4c    cd 6f 1e    . o .          (flow from: 7f4a)
l7f4fh:
    jr nz,l7f54h               ;7f4f    20 03     .                (flow from: 7f4e)
    call 018fch                ;7f51    cd fc 18    . . .          (flow from: 7f4f)
l7f54h:
    ld hl,01915h               ;7f54    21 15 19    ! . . 
    ld de,050e1h               ;7f57    11 e1 50    . . P          (flow from: 7f56)
    ld bc,0000ah               ;7f5a    01 0a 00    . . .          (flow from: 7f57)
    ldir                       ;7f5d    ed b0   . . 
    pop af                     ;7f5f    f1      . 
    jr z,l7f6eh                ;7f60    28 0c   ( .                (flow from: 7f5e)
    ld hl,(02a17h)             ;7f62    2a 17 2a    * . *          (flow from: 7f60)
...
l7f6eh:
    ...
    ex de,hl                   ;7f71    eb      . 
    call 02488h                ;7f72    cd 88 24    . . $          (flow from: 7f5a 84e7 84f7 84fd 851c)
l7f75h:
    push de                    ;7f75    d5      .                  (flow from: 7f72)
    ld (0192bh),de             ;7f76    ed 53 2b 19     . S + . 
    or a                       ;7f7a    b7      . 
    sbc hl,de                  ;7f7b    ed 52   . R                (flow from: 84e7 84f7 84fd 851c)
    push hl                    ;7f7d    e5      . 
    ld (0192eh),hl             ;7f7e    22 2e 19    " . .          (flow from: 7f7b)
    ld (050efh),hl             ;7f81    22 ef 50    " . P 
    ld hl,(02a5ah)             ;7f84    2a 5a 2a    * Z *          (flow from: 7f82)
    ld de,(02a17h)             ;7f87    ed 5b 17 2a     . [ . *    (flow from: 84d5)
    and a                      ;7f8b    a7      . 
    sbc hl,de                  ;7f8c    ed 52   . R                (flow from: 7f8a)
    ld (018c1h),de             ;7f8e    ed 53 c1 18     . S . . 
    inc hl                     ;7f92    23      #                  (flow from: 7f90 7f96)
    ld (018c6h),hl             ;7f93    22 c6 18    " . . 
    dec hl                     ;7f96    2b      +                  (flow from: 7f94)
    pop de                     ;7f97    d1      . 
    push de                    ;7f98    d5      .                  (flow from: 7f96)
    add hl,de                  ;7f99    19      . 
    inc hl                     ;7f9a    23      #                  (flow from: 7f98)
    inc hl                     ;7f9b    23      #                  (flow from: 7f9a)
    ld (050ebh),hl             ;7f9c    22 eb 50    " . P 
    ld a,00ch                  ;7f9f    3e 0c   > . 
    push ix                    ;7fa1    dd e5   . .                (flow from: 7f9e 7fac)
    call 02c34h                ;7fa3    cd 34 2c    . 4 ,          (flow from: 7fa2 7fb2 7fb8)
    pop ix                     ;7fa6    dd e1   . .                (flow from: 7fa5)
    call 018dah                ;7fa8    cd da 18    . . . 
    pop de                     ;7fab    d1      . 
    pop ix                     ;7fac    dd e1   . .                (flow from: 7fa9)
    ld a,0ffh                  ;7fae    3e ff   > .                (flow from: 7fac)
    call 004c6h                ;7fb0    cd c6 04    . . .          (flow from: 7faf)
    ld ix,00000h               ;7fb3    dd 21 00 00     . ! . . 
    dec ix                     ;7fb7    dd 2b   . +                (flow from: 7fb5)
    ld de,00000h               ;7fb9    11 00 00    . . . 
    ld l,0ffh                  ;7fbc    2e ff   . .                (flow from: 7fbb)
    ld a,h                     ;7fbe    7c      |                  (flow from: 7fbd)
    xor l                      ;7fbf    ad      .                  (flow from: 7fbe)
    ld h,a                     ;7fc0    67      g                  (flow from: 7fbf)
    ld a,001h                  ;7fc1    3e 01   > . 
    scf                        ;7fc3    37      7                  (flow from: 7fc2)
    rl l                       ;7fc4    cb 15   . . 
...
l7fe4h:
    ...
    dec d                      ;7fe5    15      . 
    inc d                      ;7fe6    14      . 
    jr nz,l7fe4h               ;7fe7    20 fb     .                (flow from: 7fda)
    ret                        ;7fe9    c9      . 
    ld b,03ah                  ;7fea    06 3a   . :                (flow from: 7fe7)
    call 01e6ch                ;7fec    cd 6c 1e    . l .          (flow from: 7fea)
    ret nz                     ;7fef    c0      .                  (flow from: 7fde 7fec 803f 83fe)
    ld de,01915h               ;7ff0    11 15 19    . . .          (flow from: 7fef)
    ld b,00ah                  ;7ff3    06 0a   . .                (flow from: 7ff1)
l7ff5h:
    call 027eeh                ;7ff5    cd ee 27    . . '          (flow from: 7ff3)
    inc hl                     ;7ff8    23      # 
    or a                       ;7ff9    b7      . 
    jr z,l8002h                ;7ffa    28 06   ( .                (flow from: 7ff7)
    ld (de),a                  ;7ffc    12      . 
    inc de                     ;7ffd    13      .                  (flow from: 7ffb)
    dec b                      ;7ffe    05      . 
    jr nz,l7ff5h               ;7fff    20 f4     . 
    ret                        ;8001    c9      .                  (flow from: 7f38)
l8002h:
    ld a,020h                  ;8002    3e 20   >                  (flow from: 8001)
l8004h:
    ld (de),a                  ;8004    12      .                  (flow from: 8002)
    inc de                     ;8005    13      .                  (flow from: 8004)
    djnz l8004h                ;8006    10 fc   . . 
    ret                        ;8008    c9      . 
    ld (hl),b                  ;8009    70      p 
    ld (hl),d                  ;800a    72      r                  (flow from: 8309 831b)
    ld l,a                     ;800b    6f      o                  (flow from: 800a)
    ld l,l                     ;800c    6d      m 
    ld h,l                     ;800d    65      e 
    ld (hl),h                  ;800e    74      t                  (flow from: 800b)
    ld l,b                     ;800f    68      h                  (flow from: 800e)
    ld h,l                     ;8010    65      e                  (flow from: 800f 8013)
sub_8011h:
    ld (hl),l                  ;8011    75      u                  (flow from: 8010)
    ld (hl),e                  ;8012    73      s                  (flow from: 8011)
l8013h:
    call 01e1ch                ;8013    cd 1c 1e    . . .          (flow from: 8012)
    call 01e5ah                ;8016    cd 5a 1e    . Z . 
    jr nz,l8013h               ;8019    20 f8     .                (flow from: 8018)
    xor a                      ;801b    af      . 
    dec a                      ;801c    3d      =                  (flow from: 801a)
    ld ix,00000h               ;801d    dd 21 00 00     . ! . . 
    ld de,00000h               ;8021    11 00 00    . . .          (flow from: 8020 8028)
    call 0194ah                ;8024    cd 4a 19    . J .          (flow from: 8022)
    ld ix,(018c1h)             ;8027    dd 2a c1 18     . * . .    (flow from: 8026)
    ld de,(018c6h)             ;802b    ed 5b c6 18     . [ . . 
    dec de                     ;802f    1b      .                  (flow from: 802d)
    ld b,0b0h                  ;8030    06 b0   . .                (flow from: 802f)
    ex af,af'                  ;8032    08      . 
    xor a                      ;8033    af      .                  (flow from: 8031)
    dec a                      ;8034    3d      = 
    ex af,af'                  ;8035    08      .                  (flow from: 8033)
    call 005c8h                ;8036    cd c8 05    . . . 
    call 01dfch                ;8039    cd fc 1d    . . . 
    jr l8041h                  ;803c    18 03   . . 
    call 01df1h                ;803e    cd f1 1d    . . .          (flow from: 803d)
l8041h:
    ret c                      ;8041    d8      .                  (flow from: 803f)
    ld a,00dh                  ;8042    3e 0d   > . 
l8044h:
    call 02c34h                ;8044    cd 34 2c    . 4 , 
    jp 022e2h                  ;8047    c3 e2 22    . . "          (flow from: 8045)
...
l8053h:
    ...
    ld a,021h                  ;8069    3e 21   > !                (flow from: 8068)
    ld hl,00000h               ;806b    21 00 00    ! . . 
    call 01a0ah                ;806e    cd 0a 1a    . . .          (flow from: 806d)
    call 02375h                ;8071    cd 75 23    . u # 
    ld a,02ah                  ;8074    3e 2a   > *                (flow from: 8070)
    ld hl,02a17h               ;8076    21 17 2a    ! . *          (flow from: 8074)
...
l80b8h:
    ...
    ld de,02d3fh               ;80ba    11 3f 2d    . ? -          (flow from: 8607)
...
l80c2h:
    ...
    cp 020h                    ;80cc    fe 20   .                  (flow from: 80bc)
    jr nc,l80d2h               ;80ce    30 02   0 . 
    ld a,020h                  ;80d0    3e 20   >   
l80d2h:
    and 07fh                   ;80d2    e6 7f   .                 (flow from: 80d1)
    ld (de),a                  ;80d4    12      .                  (flow from: 80d2)
    inc de                     ;80d5    13      .                  (flow from: 80d4)
    inc b                      ;80d6    04      . 
    jr l80c2h                  ;80d7    18 e9   . .                (flow from: 80d5 80de)
l80d9h:
    ld (01960h),hl             ;80d9    22 60 19    " ` . 
    ld a,001h                  ;80dc    3e 01   > . 
    ld (de),a                  ;80de    12      .                  (flow from: 80dd)
l80dfh:
    inc de                     ;80df    13      . 
    xor a                      ;80e0    af      .                  (flow from: 80de)
    ld (de),a                  ;80e1    12      . 
    inc b                      ;80e2    04      . 
    bit 5,b                    ;80e3    cb 68   . h 
    jr z,l80dfh                ;80e5    28 f8   ( . 
    call 0251bh                ;80e7    cd 1b 25    . . %          (flow from: 80f8)
    jp 0201dh                  ;80ea    c3 1d 20    . .            (flow from: 80e7)
    call 024a2h                ;80ed    cd a2 24    . . $          (flow from: 80ea)
    ld hl,(01960h)             ;80f0    2a 60 19    * ` . 
    ld de,(01b86h)             ;80f3    ed 5b 86 1b     . [ . .    (flow from: 80f2)
    and a                      ;80f7    a7      . 
    sbc hl,de                  ;80f8    ed 52   . R                (flow from: 80f6)
    jr c,l80b8h                ;80fa    38 bc   8 . 
    jr l80a8h                  ;80fc    18 aa   . .                (flow from: 80fb)
    ld (02356h),a              ;80fe    32 56 23    2 V #          (flow from: 80fd)
    ld (02357h),hl             ;8101    22 57 23    " W # 
    ret                        ;8104    c9      . 
l8105h:
    call 01e1ch                ;8105    cd 1c 1e    . . .          (flow from: 8103)
    call 01e4dh                ;8108    cd 4d 1e    . M .          (flow from: 8103 8106)
    jr nz,l8105h               ;810b    20 f8     . 
    ld hl,(050ebh)             ;810d    2a eb 50    * . P 
    ld b,h                     ;8110    44      D 
    ld c,l                     ;8111    4d      M 
    call 022a8h                ;8112    cd a8 22    . . "          (flow from: 810f)
    ex de,hl                   ;8115    eb      . 
    ld hl,(01b86h)             ;8116    2a 86 1b    * . .          (flow from: 8112 821f)
    and a                      ;8119    a7      .                  (flow from: 8116)
    sbc hl,de                  ;811a    ed 52   . R                (flow from: 8119)
    push hl                    ;811c    e5      .                  (flow from: 811a)
    ld (01960h),hl             ;811d    22 60 19    " ` .          (flow from: 811c)
    ld bc,(050efh)             ;8120    ed 4b ef 50     . K . P    (flow from: 811f)
    add hl,bc                  ;8124    09      .                  (flow from: 8123)
    ld (0196eh),hl             ;8125    22 6e 19    " n .          (flow from: 8124)
    inc hl                     ;8128    23      #                  (flow from: 8127)
    inc hl                     ;8129    23      #                  (flow from: 8128)
    ld (01978h),hl             ;812a    22 78 19    " x .          (flow from: 8129)
    pop ix                     ;812d    dd e1   . .                (flow from: 812c)
    scf                        ;812f    37      7                  (flow from: 812e)
    sbc a,a                    ;8130    9f      .                  (flow from: 812f)
    call 0194ah                ;8131    cd 4a 19    . J .          (flow from: 8130)
    ld a,007h                  ;8134    3e 07   > .                (flow from: 8133)
    out (0feh),a               ;8136    d3 fe   . . 
    ret                        ;8138    c9      .                  (flow from: 7d20 8135)
    call 01e6ah                ;8139    cd 6a 1e    . j .          (flow from: 8138)
    ld a,000h                  ;813c    3e 00   > .                (flow from: 8139)
    jr z,l8141h                ;813e    28 01   ( . 
    inc a                      ;8140    3c      <                  (flow from: 813f)
l8141h:
    ld (01a7fh),a              ;8141    32 7f 1a    2  . 
    ld c,0b6h                  ;8144    0e b6   . .                (flow from: 8143)
...
l8154h:
    ...
    ld (01b75h),hl             ;815b    22 75 1b    " u .          (flow from: 8144)
    ld hl,03e68h               ;815e    21 68 3e    ! h > 
l8161h:
    call 02c6fh                ;8161    cd 6f 2c    . o ,          (flow from: 815d)
    jr nc,l8184h               ;8164    30 1e   0 .                (flow from: 8161 82e5)
    ld (01c04h),hl             ;8166    22 04 1c    " . .          (flow from: 8164)
    push hl                    ;8169    e5      . 
    call 02488h                ;816a    cd 88 24    . . $          (flow from: 8134)
    ld (01a8ch),hl             ;816d    22 8c 1a    " . .          (flow from: 816b)
    pop ix                     ;8170    dd e1   . .                (flow from: 8164 82e5)
    ld a,000h                  ;8172    3e 00   > . 
    or a                       ;8174    b7      .                  (flow from: 8171)
    jr nz,l817ch               ;8175    20 05     . 
    call 02335h                ;8177    cd 35 23    . 5 #          (flow from: 8174)
    jr c,l817fh                ;817a    38 03   8 .                (flow from: 8179)
l817ch:
...
l8184h:
    ...
    dec a                      ;8186    3d      =                  (flow from: 817e)
    ld (01a91h),a              ;8187    32 91 1a    2 . . 
    ld hl,01b1ah               ;818a    21 1a 1b    ! . .          (flow from: 8189)
    ld (01a89h),hl             ;818d    22 89 1a    " . . 
    jr z,l8154h                ;8190    28 c2   ( .                (flow from: 818e)
    ret                        ;8192    c9      .                  (flow from: 8190)
    call 01a45h                ;8193    cd 45 1a    . E . 
    ld a,00bh                  ;8196    3e 0b   > . 
    jp 01f1ah                  ;8198    c3 1a 1f    . . .          (flow from: 8195)
l819bh:
    ld a,(ix+000h)             ;819b    dd 7e 00    . ~ .          (flow from: 82d3)
    call 01c42h                ;819e    cd 42 1c    . B . 
    sub 002h                   ;81a1    d6 02   . .                (flow from: 81a0)
    ret c                      ;81a3    d8      .                  (flow from: 81a1)
    jr nz,l81b1h               ;81a4    20 0b     .                (flow from: 81a3)
    call 01cd8h                ;81a6    cd d8 1c    . . . 
    ld (01f04h),hl             ;81a9    22 04 1f    " . . 
    ld hl,01ef9h               ;81ac    21 f9 1e    ! . .          (flow from: 81aa)
    dec (hl)                   ;81af    35      5                  (flow from: 81ae)
    ret                        ;81b0    c9      .                  (flow from: 81af)
l81b1h:
    dec a                      ;81b1    3d      = 
    ret z                      ;81b2    c8      . 
    dec a                      ;81b3    3d      =                  (flow from: 81bb 81dd 822d 8231)
    jp z,01c65h                ;81b4    ca 65 1c    . e .          (flow from: 81b3)
    dec a                      ;81b7    3d      =                  (flow from: 81b6)
    jr nz,l81c0h               ;81b8    20 06     . 
    call 01cd8h                ;81ba    cd d8 1c    . . .          (flow from: 81b0 81b9)
    jp 01c6ch                  ;81bd    c3 6c 1c    . l . 
l81c0h:
    dec a                      ;81c0    3d      =                  (flow from: 81be)
    jr nz,l81d0h               ;81c1    20 0d     . 
l81c3h:
    call 01cd8h                ;81c3    cd d8 1c    . . .          (flow from: 81c0)
    jp c,01b6ah                ;81c6    da 6a 1b    . j .          (flow from: 8223 82d3)
    call 01b6ah                ;81c9    cd 6a 1b    . j .          (flow from: 81c8)
    inc ix                     ;81cc    dd 23   . #                (flow from: 81cb)
...
l81d3h:
    ...
    jr nz,l81ddh               ;81d6    20 05     .                (flow from: 81cd)
    call 01b71h                ;81d8    cd 71 1b    . q . 
    jr l81d3h                  ;81db    18 f6   . .                (flow from: 81da)
l81ddh:
    ld a,e                     ;81dd    7b      {                  (flow from: 81db)
    cp 027h                    ;81de    fe 27   . '                (flow from: 81be)
    jr nz,l81e4h               ;81e0    20 02     . 
    set 7,d                    ;81e2    cb fa   . . 
l81e4h:
    ld a,d                     ;81e4    7a      z 
    jr l8265h                  ;81e5    18 7e   . ~                (flow from: 81e3)
...
l8201h:
    call 01cd8h                ;8201    cd d8 1c    . . .          (flow from: 81e1 81e8 8214 8226)
    jp c,01ba2h                ;8204    da a2 1b    . . .          (flow from: 8201)
    call 01ba2h                ;8207    cd a2 1b    . . . 
    inc ix                     ;820a    dd 23   . #                (flow from: 8208)
    jr l8201h                  ;820c    18 f3   . .                (flow from: 820a)
    ld b,(ix+001h)             ;820e    dd 46 01    . F .          (flow from: 820c)
    ld a,b                     ;8211    78      x                  (flow from: 8210)
    and 030h                   ;8212    e6 30   . 0                (flow from: 8211)
    cp 030h                    ;8214    fe 30   . 0                (flow from: 8212)
    jr z,l819bh                ;8216    28 83   ( .                (flow from: 820e)
    ld a,b                     ;8218    78      x 
    and 0b0h                   ;8219    e6 b0   . . 
    cp 090h                    ;821b    fe 90   . . 
    jr c,l822dh                ;821d    38 0e   8 . 
    call 01b39h                ;821f    cd 39 1b    . 9 .          (flow from: 821e)
    ld hl,(01b75h)             ;8222    2a 75 1b    * u .          (flow from: 8134)
    dec hl                     ;8225    2b      + 
    ld a,(hl)                  ;8226    7e      ~                  (flow from: 81dd)
    dec hl                     ;8227    2b      + 
    ld b,(hl)                  ;8228    46      F                  (flow from: 820a)
    ld (hl),a                  ;8229    77      w                  (flow from: 8228)
    inc hl                     ;822a    23      # 
    ld (hl),b                  ;822b    70      p                  (flow from: 8229)
    ret                        ;822c    c9      . 
l822dh:
    ld a,0ddh                  ;822d    3e dd   > .                (flow from: 822b)
    bit 5,b                    ;822f    cb 68   . h 
    call nz,01b71h             ;8231    c4 71 1b    . q .          (flow from: 8230)
    ld a,0fdh                  ;8234    3e fd   > . 
    bit 4,b                    ;8236    cb 60   . `                (flow from: 8235)
    call nz,01b71h             ;8238    c4 71 1b    . q . 
    ld a,0cbh                  ;823b    3e cb   > .                (flow from: 8239)
    bit 7,b                    ;823d    cb 78   . x                (flow from: 823b)
    call nz,01b71h             ;823f    c4 71 1b    . q . 
    ld a,0edh                  ;8242    3e ed   > .                (flow from: 8240)
    bit 6,b                    ;8244    cb 70   . p                (flow from: 8243)
    call nz,01b71h             ;8246    c4 71 1b    . q . 
    ld a,(ix+000h)             ;8249    dd 7e 00    . ~ .          (flow from: 8248)
    call 01b71h                ;824c    cd 71 1b    . q . 
    call 01c42h                ;824f    cd 42 1c    . B .          (flow from: 824d)
    ld a,b                     ;8252    78      x 
    and 007h                   ;8253    e6 07   . .                (flow from: 824d)
    ret z                      ;8255    c8      .                  (flow from: 8253)
    dec a                      ;8256    3d      =                  (flow from: 8251 8255 825d 8260)
    push af                    ;8257    f5      .                  (flow from: 8256)
    call 01cd8h                ;8258    cd d8 1c    . . .          (flow from: 8257)
    pop af                     ;825b    f1      .                  (flow from: 825a)
    jr nz,l8293h               ;825c    20 35     5 
l825eh:
    ld a,b                     ;825e    78      x 
    inc a                      ;825f    3c      <                  (flow from: 825d)
    and 0feh                   ;8260    e6 fe   . .                (flow from: 825f)
    jr nz,l82bbh               ;8262    20 57     W                (flow from: 7760)
    ld a,c                     ;8264    79      y 
l8265h:
    ld de,00000h               ;8265    11 00 00    . . .          (flow from: 8263)
    ld hl,00000h               ;8268    21 00 00    ! . .          (flow from: 8266)
    and a                      ;826b    a7      . 
    sbc hl,de                  ;826c    ed 52   . R                (flow from: 7cf1)
    add hl,de                  ;826e    19      . 
    ex de,hl                   ;826f    eb      .                  (flow from: 826c)
    jr c,l8279h                ;8270    38 07   8 . 
    ld hl,(02a5ah)             ;8272    2a 5a 2a    * Z * 
    sbc hl,de                  ;8275    ed 52   . R 
    jr nc,l828fh               ;8277    30 16   0 . 
l8279h:
    ld hl,0ffffh               ;8279    21 ff ff    ! . .          (flow from: 8278)
    and a                      ;827c    a7      . 
    sbc hl,de                  ;827d    ed 52   . R 
    jr c,l828fh                ;827f    38 0e   8 . 
    ld (de),a                  ;8281    12      . 
    inc de                     ;8282    13      . 
    ld (01b75h),de             ;8283    ed 53 75 1b     . S u .    (flow from: 82b1)
    ld hl,00000h               ;8287    21 00 00    ! . .          (flow from: 8255 825a)
    inc hl                     ;828a    23      #                  (flow from: 8288)
    ld (01b94h),hl             ;828b    22 94 1b    " . . 
    ret                        ;828e    c9      .                  (flow from: 828c)
l828fh:
    ld a,008h                  ;828f    3e 08   > .                (flow from: 8280)
    jr l82f7h                  ;8291    18 64   . d                (flow from: 828f)
l8293h:
    dec a                      ;8293    3d      = 
    jr nz,l829dh               ;8294    20 07     .                (flow from: 8189 81b6)
    ld a,c                     ;8296    79      y                  (flow from: 8294)
    call 01b71h                ;8297    cd 71 1b    . q . 
    ld a,b                     ;829a    78      x 
    jr l8265h                  ;829b    18 c8   . .                (flow from: 8299)
l829dh:
    dec a                      ;829d    3d      = 
    jr nz,l82bfh               ;829e    20 1f     .                (flow from: 829b)
    ld hl,(01b94h)             ;82a0    2a 94 1b    * . .          (flow from: 8299 829e)
    inc hl                     ;82a3    23      #                  (flow from: 82a0)
    push bc                    ;82a4    c5      . 
    ex (sp),hl                 ;82a5    e3      . 
    pop bc                     ;82a6    c1      .                  (flow from: 82a3)
    and a                      ;82a7    a7      . 
    sbc hl,bc                  ;82a8    ed 42   . B 
l82aah:
    ld a,l                     ;82aa    7d      } 
    inc h                      ;82ab    24      $                  (flow from: 82a9 82ae 82c7)
    jr z,l82b7h                ;82ac    28 09   ( .                (flow from: 82ab)
    dec h                      ;82ae    25      %                  (flow from: 82ad)
    jr nz,l82bbh               ;82af    20 0a     . 
    cp 080h                    ;82b1    fe 80   . .                (flow from: 82b0)
    jr nc,l82bbh               ;82b3    30 06   0 . 
l82b5h:
    jr l8265h                  ;82b5    18 ae   . . 
l82b7h:
    cp 080h                    ;82b7    fe 80   . . 
    jr nc,l82b5h               ;82b9    30 fa   0 .                (flow from: 82b4)
l82bbh:
    ld a,003h                  ;82bb    3e 03   > .                (flow from: 82b9)
    jr l82f7h                  ;82bd    18 38   . 8                (flow from: 82bb)
l82bfh:
    dec a                      ;82bf    3d      = 
    jr nz,l82c6h               ;82c0    20 04     .                (flow from: 8730 873c)
    ld h,b                     ;82c2    60      `                  (flow from: 82c0)
    ld l,c                     ;82c3    69      i 
    jr l82aah                  ;82c4    18 e4   . .                (flow from: 82c0 82c2)
l82c6h:
    dec a                      ;82c6    3d      = 
    jr nz,l82d3h               ;82c7    20 0a     .                (flow from: 8749)
    call 01bceh                ;82c9    cd ce 1b    . . .          (flow from: 8198 81c3)
    inc ix                     ;82cc    dd 23   . # 
    call 01cd8h                ;82ce    cd d8 1c    . . .          (flow from: 82cd)
    jr l825eh                  ;82d1    18 8b   . .                (flow from: 82d0)
l82d3h:
    ld a,c                     ;82d3    79      y                  (flow from: 82d2)
    and 0c7h                   ;82d4    e6 c7   . .                (flow from: 816d 819d)
    or b                       ;82d6    b0      . 
    ld a,006h                  ;82d7    3e 06   > .                (flow from: 8161 81dd)
    jr nz,l82f7h               ;82d9    20 1c     .                (flow from: 82d7)
    ld a,c                     ;82db    79      y                  (flow from: 775a)
    ld hl,(01b75h)             ;82dc    2a 75 1b    * u . 
    dec hl                     ;82df    2b      + 
    add a,(hl)                 ;82e0    86      . 
    ld (hl),a                  ;82e1    77      w                  (flow from: 1 7cd5 7ce3 7d1c 7de3 813c 82d9 82de 82e3)
    ret                        ;82e2    c9      .                  (flow from: 82e1)
    bit 3,(ix+001h)            ;82e3    dd cb 01 5e     . . . ^    (flow from: 82e2)
    jr z,l830ah                ;82e7    28 21   ( ! 
    call 02353h                ;82e9    cd 53 23    . S # 
    ld (01c5bh),de             ;82ec    ed 53 5b 1c     . S [ . 
    ld a,(hl)                  ;82f0    7e      ~                  (flow from: 82ee)
    and 0c0h                   ;82f1    e6 c0   . . 
    jr z,l8300h                ;82f3    28 0b   ( . 
    ld a,012h                  ;82f5    3e 12   > . 
l82f7h:
    ld hl,00000h               ;82f7    21 00 00    ! . . 
    ld (024adh),hl             ;82fa    22 ad 24    " . $          (flow from: 82f9)
    jp 022dah                  ;82fd    c3 da 22    . . " 
l8300h:
    set 6,(hl)                 ;8300    cb f6   . . 
    ld hl,(01b94h)             ;8302    2a 94 1b    * . . 
    ex de,hl                   ;8305    eb      .                  (flow from: 8304)
    dec hl                     ;8306    2b      + 
    ld (hl),d                  ;8307    72      r 
    dec hl                     ;8308    2b      + 
    ld (hl),e                  ;8309    73      s                  (flow from: 8305)
l830ah:
    ld a,(ix+001h)             ;830a    dd 7e 01    . ~ .          (flow from: 8309)
    and 030h                   ;830d    e6 30   . 0                (flow from: 8818 88e0)
    cp 030h                    ;830f    fe 30   . 0 
    jr z,l8340h                ;8311    28 2d   ( -                (flow from: 830d)
    ld a,(ix+001h)             ;8313    dd 7e 01    . ~ .          (flow from: 8311)
    and 007h                   ;8316    e6 07   . .                (flow from: 8313)
    ld c,a                     ;8318    4f      O 
    ld b,000h                  ;8319    06 00   . .                (flow from: 8316)
    ld hl,01c3ah               ;831b    21 3a 1c    ! : .          (flow from: 8319)
    add hl,bc                  ;831e    09      . 
    ld a,(hl)                  ;831f    7e      ~                  (flow from: 831c)
    inc a                      ;8320    3c      < 
    ld bc,00400h               ;8321    01 00 04    . . .          (flow from: 831f)
    ld d,(ix+001h)             ;8324    dd 56 01    . V . 
l8327h:
    rl d                       ;8327    cb 12   . .                (flow from: 8325)
    adc a,c                    ;8329    89      .                  (flow from: 8327)
    djnz l8327h                ;832a    10 fb   . .                (flow from: 8329)
    jr l839bh                  ;832c    18 6d   . m                (flow from: 832a)
    nop                        ;832e    00      .                  (flow from: 8323 8327 832c)
    ld bc,00102h               ;832f    01 02 01    . . . 
    ld bc,00002h               ;8332    01 02 00    . . . 
    nop                        ;8335    00      .                  (flow from: 8334)
    bit 3,(ix+001h)            ;8336    dd cb 01 5e     . . . ^ 
    ret z                      ;833a    c8      .                  (flow from: 8339)
    inc ix                     ;833b    dd 23   . # 
    inc ix                     ;833d    dd 23   . #                (flow from: 833c)
    ret                        ;833f    c9      .                  (flow from: 833e)
l8340h:
    ld a,(ix+000h)             ;8340    dd 7e 00    . ~ . 
    call 01c42h                ;8343    cd 42 1c    . B . 
    sub 003h                   ;8346    d6 03   . .                (flow from: 8344)
    ret c                      ;8348    d8      .                  (flow from: 8346)
    jr nz,l8356h               ;8349    20 0b     . 
    call 01cd8h                ;834b    cd d8 1c    . . .          (flow from: 8348 834a)
    ld hl,00000h               ;834e    21 00 00    ! . . 
    dec hl                     ;8351    2b      +                  (flow from: 834f)
    ld (hl),b                  ;8352    70      p                  (flow from: 8351)
    dec hl                     ;8353    2b      +                  (flow from: 8352)
    ld (hl),c                  ;8354    71      q                  (flow from: 834f 8353)
    ret                        ;8355    c9      .                  (flow from: 8354)
l8356h:
    dec a                      ;8356    3d      = 
    jr nz,l8365h               ;8357    20 0c     . 
    call 01cd8h                ;8359    cd d8 1c    . . . 
    ld (01b94h),bc             ;835c    ed 43 94 1b     . C . .    (flow from: 835b)
    ld (01b75h),bc             ;8360    ed 43 75 1b     . C u .    (flow from: 835f)
    ret                        ;8364    c9      . 
l8365h:
    dec a                      ;8365    3d      =                  (flow from: 8363)
    ret z                      ;8366    c8      . 
    dec a                      ;8367    3d      =                  (flow from: 8365)
    jr nz,l8370h               ;8368    20 06     . 
    call 01cb0h                ;836a    cd b0 1c    . . . 
    ld a,c                     ;836d    79      y                  (flow from: 836b)
    jr l839bh                  ;836e    18 2b   . + 
l8370h:
    dec a                      ;8370    3d      = 
    jr nz,l837dh               ;8371    20 0a     .                (flow from: 836f)
    ld c,a                     ;8373    4f      O                  (flow from: 8371)
l8374h:
    inc c                      ;8374    0c      . 
    call 01af3h                ;8375    cd f3 1a    . . .          (flow from: 8373)
    jr z,l8374h                ;8378    28 fa   ( .                (flow from: 8377)
    ld a,c                     ;837a    79      y                  (flow from: 8379)
    jr l839bh                  ;837b    18 1e   . . 
l837dh:
    dec a                      ;837d    3d      =                  (flow from: 84d5)
    jr nz,l8396h               ;837e    20 16     . 
l8380h:
    call 01cd8h                ;8380    cd d8 1c    . . .          (flow from: 837d)
    push af                    ;8383    f5      . 
    ld hl,01b94h               ;8384    21 94 1b    ! . . 
    call 02bc0h                ;8387    cd c0 2b    . . +          (flow from: 8385)
    ld hl,01b75h               ;838a    21 75 1b    ! u .          (flow from: 8389)
    call 02bc0h                ;838d    cd c0 2b    . . + 
    pop af                     ;8390    f1      .                  (flow from: 838e)
    inc ix                     ;8391    dd 23   . # 
    jr nc,l8380h               ;8393    30 eb   0 .                (flow from: 8392 8398)
    ret                        ;8395    c9      .                  (flow from: 8394)
l8396h:
    call 01cb0h                ;8396    cd b0 1c    . . . 
    ld a,c                     ;8399    79      y 
    add a,c                    ;839a    81      .                  (flow from: 8398)
l839bh:
    ld b,000h                  ;839b    06 00   . . 
    ld c,a                     ;839d    4f      O                  (flow from: 839c)
    ld hl,01b94h               ;839e    21 94 1b    ! . . 
    jp 02bc0h                  ;83a1    c3 c0 2b    . . + 
    ld c,001h                  ;83a4    0e 01   . .                (flow from: 83a2)
l83a6h:
    ld a,(ix+002h)             ;83a6    dd 7e 02    . ~ . 
    cp 02ch                    ;83a9    fe 2c   . , 
    jr nz,l83b0h               ;83ab    20 03     .                (flow from: 83a2 83a7)
    inc c                      ;83ad    0c      . 
    jr l83bdh                  ;83ae    18 0d   . .                (flow from: 8417 849d)
l83b0h:
    cp 022h                    ;83b0    fe 22   . " 
...
l83c1h:
    cp 0c0h                    ;83c1    fe c0   . .                (flow from: 83b7)
    ret nc                     ;83c3    d0      .                  (flow from: 83c1)
    cp 080h                    ;83c4    fe 80   . . 
    jr c,l83bdh                ;83c6    38 f5   8 . 
    inc ix                     ;83c8    dd 23   . #                (flow from: 849f)
    jr l83bdh                  ;83ca    18 f1   . .                (flow from: 83c3)
    ld hl,00000h               ;83cc    21 00 00    ! . . 
    ld a,02bh                  ;83cf    3e 2b   > + 
    push hl                    ;83d1    e5      . 
    push af                    ;83d2    f5      .                  (flow from: 83d0)
    ld a,(ix+002h)             ;83d3    dd 7e 02    . ~ .          (flow from: 83d2)
    push af                    ;83d6    f5      . 
    cp 02dh                    ;83d7    fe 2d   . -                (flow from: 83d5)
    jr nz,l83ddh               ;83d9    20 02     . 
    inc ix                     ;83db    dd 23   . #                (flow from: 8818 88e0)
l83ddh:
    ld a,(ix+002h)             ;83dd    dd 7e 02    . ~ .          (flow from: 83db)
    cp 024h                    ;83e0    fe 24   . $ 
    ld de,(01b94h)             ;83e2    ed 5b 94 1b     . [ . .    (flow from: 83df)
    jr z,l8409h                ;83e6    28 21   ( ! 
    cp 080h                    ;83e8    fe 80   . . 
    jp c,01d9bh                ;83ea    da 9b 1d    . . .          (flow from: 83e9)
    ld d,a                     ;83ed    57      W                  (flow from: 83ea)
    ld e,(ix+003h)             ;83ee    dd 5e 03    . ^ . 
    inc ix                     ;83f1    dd 23   . #                (flow from: 83f0)
    call 02356h                ;83f3    cd 56 23    . V #          (flow from: 83f2)
    ld a,(hl)                  ;83f6    7e      ~ 
    and 0c0h                   ;83f7    e6 c0   . .                (flow from: 849d 849f)
    jr nz,l8404h               ;83f9    20 09     .                (flow from: 83d0)
    ld (02300h),de             ;83fb    ed 53 00 23     . S . # 
    ld a,009h                  ;83ff    3e 09   > .                (flow from: 83fe)
    jp 01c03h                  ;8401    c3 03 1c    . . .          (flow from: 83ff)
l8404h:
    dec de                     ;8404    1b      . 
    ex de,hl                   ;8405    eb      . 
    ld d,(hl)                  ;8406    56      V 
...
l8409h:
    ...
    ld b,h                     ;8412    44      D 
    ld c,l                     ;8413    4d      M 
    ret z                      ;8414    c8      .                  (flow from: 8401)
    xor 033h                   ;8415    ee 33   . 3 
    ret z                      ;8417    c8      .                  (flow from: 849d)
    cp 0c0h                    ;8418    fe c0   . . 
    ccf                        ;841a    3f      ? 
    ret c                      ;841b    d8      .                  (flow from: 8419)
    xor 01fh                   ;841c    ee 1f   . . 
    inc ix                     ;841e    dd 23   . # 
...
l8437h:
    ex de,hl                   ;8437    eb      . 
    ex de,hl                   ;8438    eb      .                  (flow from: 8422)
l8439h:
    pop af                     ;8439    f1      . 
    call 01d90h                ;843a    cd 90 1d    . . . 
    pop af                     ;843d    f1      . 
    pop hl                     ;843e    e1      .                  (flow from: 84a7)
    ld bc,01d19h               ;843f    01 19 1d    . . .          (flow from: 843e)
    push bc                    ;8442    c5      .                  (flow from: 8441)
    cp 02bh                    ;8443    fe 2b   . +                (flow from: 8442)
    jr z,l847fh                ;8445    28 38   ( 8                (flow from: 8444 844a)
    cp 02dh                    ;8447    fe 2d   . - 
    jr z,l8481h                ;8449    28 36   ( 6                (flow from: 8446)
    cp 02ah                    ;844b    fe 2a   . * 
    jr z,l846eh                ;844d    28 1f   ( .                (flow from: 844c)
    cp 02fh                    ;844f    fe 2f   . / 
    jr z,l8468h                ;8451    28 15   ( .                (flow from: 844e)
    ld a,h                     ;8453    7c      |                  (flow from: 8451)
    ld c,l                     ;8454    4d      M                  (flow from: 8453)
    ld hl,00000h               ;8455    21 00 00    ! . .          (flow from: 8454)
    ld b,010h                  ;8458    06 10   . .                (flow from: 8457)
l845ah:
    sli c                      ;845a    cb 31   . 1 
    rla                        ;845c    17      .                  (flow from: 849d 849f)
    adc hl,hl                  ;845d    ed 6a   . j                (flow from: 845c)
...
l8481h:
    ...
    inc de                     ;848d    13      . 
    ret                        ;848e    c9      . 
    ld a,(ix+002h)             ;848f    dd 7e 02    . ~ .          (flow from: 83c5 8414 8459)
    cp 022h                    ;8492    fe 22   . "                (flow from: 83ab 83f4 8968)
    jr z,l8423h                ;8494    28 8d   ( .                (flow from: 8493)
    ld c,00ah                  ;8496    0e 0a   . .                (flow from: 8494)
    cp 025h                    ;8498    fe 25   . %                (flow from: 8496)
    jr nz,l84a0h               ;849a    20 04     .                (flow from: 8498)
    inc ix                     ;849c    dd 23   . #                (flow from: 849a)
    ld c,002h                  ;849e    0e 02   . .                (flow from: 8496 849a 849d)
l84a0h:
    cp 023h                    ;84a0    fe 23   . #                (flow from: 843b)
    jr nz,l84a8h               ;84a2    20 04     .                (flow from: 84a1)
    inc ix                     ;84a4    dd 23   . # 
...
l84bbh:
    ...
    dec a                      ;84c1    3d      = 
    ld d,h                     ;84c2    54      T 
    ld e,l                     ;84c3    5d      ]                  (flow from: 7f84 7f8c 837a 8382)
l84c4h:
    add hl,de                  ;84c4    19      . 
    dec a                      ;84c5    3d      =                  (flow from: 84c3)
    jr nz,l84c4h               ;84c6    20 fc     . 
    pop af                     ;84c8    f1      . 
    ld d,000h                  ;84c9    16 00   . .                (flow from: 84c7)
    ld e,a                     ;84cb    5f      _                  (flow from: 84c9)
    add hl,de                  ;84cc    19      . 
    inc ix                     ;84cd    dd 23   . #                (flow from: 84cb)
    jr l84abh                  ;84cf    18 da   . .                (flow from: 84cd)
    ld a,(ix+002h)             ;84d1    dd 7e 02    . ~ .          (flow from: 84cf)
    cp 022h                    ;84d4    fe 22   . " 
    jr nz,l84e1h               ;84d6    20 09     .                (flow from: 84c9)
    cp (ix+003h)               ;84d8    dd be 03    . . .          (flow from: 84d7)
    inc ix                     ;84db    dd 23   . #                (flow from: 84d8)
    jr z,l84e1h                ;84dd    28 02   ( .                (flow from: 84db)
    scf                        ;84df    37      7                  (flow from: 84de)
    ret                        ;84e0    c9      .                  (flow from: 7f6f 7f78)
l84e1h:
    inc ix                     ;84e1    dd 23   . #                (flow from: 84e0)
    or a                       ;84e3    b7      . 
    ret                        ;84e4    c9      .                  (flow from: 8522)
    inc d                      ;84e5    14      .                  (flow from: 84e4)
    ex af,af'                  ;84e6    08      .                  (flow from: 84e5)
    dec d                      ;84e7    15      .                  (flow from: 84e6)
    di                         ;84e8    f3      .                  (flow from: 84e7)
    ld a,00fh                  ;84e9    3e 0f   > . 
    out (0feh),a               ;84eb    d3 fe   . . 
    call 00562h                ;84ed    cd 62 05    . b .          (flow from: 84ec)
    push af                    ;84f0    f5      .                  (flow from: 84ed)
    ld a,07fh                  ;84f1    3e 7f   >  
    in a,(0feh)                ;84f3    db fe   . .                (flow from: 856e)
    rra                        ;84f5    1f      . 
    jp nc,022e2h               ;84f6    d2 e2 22    . . " 
    pop af                     ;84f9    f1      .                  (flow from: 84f8)
    ret                        ;84fa    c9      . 
    ld b,061h                  ;84fb    06 61   . a                (flow from: 84f9)
    call 01e6ch                ;84fd    cd 6c 1e    . l .          (flow from: 84fb)
    call z,01a45h              ;8500    cc 45 1a    . E .          (flow from: 84ff)
...
l8510h:
    ...
    jr nc,l8510h               ;851c    30 f2   0 .                (flow from: 851a)
    ld a,011h                  ;851e    3e 11   > . 
    call 022f4h                ;8520    cd f4 22    . . "          (flow from: 851f)
    ld hl,050e1h               ;8523    21 e1 50    ! . P          (flow from: 8522)
    ld b,00ah                  ;8526    06 0a   . . 
l8528h:
    ld a,(hl)                  ;8528    7e      ~                  (flow from: 8527 8533)
    cp 020h                    ;8529    fe 20   .                  (flow from: 8528)
    jr c,l8531h                ;852b    38 04   8 .                (flow from: 8529)
    cp 080h                    ;852d    fe 80   . . 
    jr c,l8533h                ;852f    38 02   8 .                (flow from: 852e)
l8531h:
    ld a,03fh                  ;8531    3e 3f   > ?                (flow from: 8530)
l8533h:
    call 02984h                ;8533    cd 84 29    . . )          (flow from: 8531)
    inc hl                     ;8536    23      #                  (flow from: 8535)
    djnz l8528h                ;8537    10 ef   . .                (flow from: 8536)
    ld a,(050e0h)              ;8539    3a e0 50    : . P          (flow from: 852c)
    cp 003h                    ;853c    fe 03   . . 
    jr nz,l8510h               ;853e    20 d0     .                (flow from: 853d)
    ret                        ;8540    c9      .                  (flow from: 853e)
    ld b,00ah                  ;8541    06 0a   . .                (flow from: 8540)
    ld hl,01915h               ;8543    21 15 19    ! . .          (flow from: 8541)
l8546h:
    ld a,(hl)                  ;8546    7e      ~ 
    cp 020h                    ;8547    fe 20   .   
...
l8556h:
    ...
    cp (hl)                    ;8557    be      . 
    inc hl                     ;8558    23      # 
    inc de                     ;8559    13      .                  (flow from: 7f25 84f0)
    ret nz                     ;855a    c0      .                  (flow from: 8559)
    djnz l8556h                ;855b    10 f9   . .                (flow from: 855a)
    ret                        ;855d    c9      .                  (flow from: 855c)
    ld b,042h                  ;855e    06 42   . B                (flow from: 855d)
    ld hl,02d40h               ;8560    21 40 2d    ! @ -          (flow from: 855f)
    call 027eeh                ;8563    cd ee 27    . . '          (flow from: 8562)
    inc hl                     ;8566    23      #                  (flow from: 8565)
    cp b                       ;8567    b8      .                  (flow from: 8566)
    ret z                      ;8568    c8      .                  (flow from: 8567)
    set 5,b                    ;8569    cb e8   . .                (flow from: 8568)
    cp b                       ;856b    b8      .                  (flow from: 856a)
    ret                        ;856c    c9      .                  (flow from: 856b)
    ld hl,02b49h               ;856d    21 49 2b    ! I +          (flow from: 856c)
    jr l8575h                  ;8570    18 03   . . 
    ld hl,0205fh               ;8572    21 5f 20    ! _   
l8575h:
    ld a,(hl)                  ;8575    7e      ~                  (flow from: 8573)
    xor 001h                   ;8576    ee 01   . . 
    ld (hl),a                  ;8578    77      w 
    ret                        ;8579    c9      .                  (flow from: 8571 8575 8577 8597)
    ld iy,05c3ah               ;857a    fd 21 3a 5c     . ! : \ 
    im 1                       ;857e    ed 56   . V                (flow from: 857c)
    ei                         ;8580    fb      .                  (flow from: 857e)
    ld sp,(05c3dh)             ;8581    ed 7b 3d 5c     . { = \ 
    jp 01b76h                  ;8585    c3 76 1b    . v .          (flow from: 8584)
    push af                    ;8588    f5      .                  (flow from: 8586)
    ld c,000h                  ;8589    0e 00   . .                (flow from: 8588)
    call 022b0h                ;858b    cd b0 22    . . "          (flow from: 8589)
    push hl                    ;858e    e5      . 
    call 01ec3h                ;858f    cd c3 1e    . . .          (flow from: 858d)
    pop de                     ;8592    d1      .                  (flow from: 8591)
    pop af                     ;8593    f1      . 
    ret                        ;8594    c9      .                  (flow from: 8592)
    push hl                    ;8595    e5      .                  (flow from: 8594)
    pop ix                     ;8596    dd e1   . .                (flow from: 8595)
    call 024cfh                ;8598    cd cf 24    . . $ 
    ld a,0efh                  ;859b    3e ef   > .                (flow from: 85a8)
    in a,(0feh)                ;859d    db fe   . . 
    ret                        ;859f    c9      .                  (flow from: 859e)
    call 02472h                ;85a0    cd 72 24    . r $ 
    call 02c64h                ;85a3    cd 64 2c    . d , 
    ccf                        ;85a6    3f      ?                  (flow from: 85a5)
    jr l85afh                  ;85a7    18 06   . .                (flow from: 85a6)
    call 02485h                ;85a9    cd 85 24    . . $ 
    call 02c6fh                ;85ac    cd 6f 2c    . o , 
l85afh:
    pop de                     ;85af    d1      .                  (flow from: 85ae)
    jr nc,l8622h               ;85b0    30 70   0 p 
    push de                    ;85b2    d5      .                  (flow from: 85b1)
    ld (024adh),hl             ;85b3    22 ad 24    " . $          (flow from: 85b2)
    ret                        ;85b6    c9      . 
    push hl                    ;85b7    e5      . 
    push de                    ;85b8    d5      .                  (flow from: 85b1 85b4)
    ld b,008h                  ;85b9    06 08   . . 
l85bbh:
    push hl                    ;85bb    e5      .                  (flow from: 85ba)
    push de                    ;85bc    d5      .                  (flow from: 85bb)
    ld c,020h                  ;85bd    0e 20   .   
l85bfh:
    ld a,(hl)                  ;85bf    7e      ~ 
    ld (de),a                  ;85c0    12      . 
    inc l                      ;85c1    2c      ,                  (flow from: 85be)
    inc e                      ;85c2    1c      . 
    dec c                      ;85c3    0d      . 
    jr nz,l85bfh               ;85c4    20 f9     .                (flow from: 85c1)
    pop de                     ;85c6    d1      . 
    pop hl                     ;85c7    e1      .                  (flow from: 85c4)
    inc h                      ;85c8    24      $ 
    inc d                      ;85c9    14      . 
    djnz l85bbh                ;85ca    10 ef   . .                (flow from: 8607)
    pop de                     ;85cc    d1      . 
    pop hl                     ;85cd    e1      .                  (flow from: 85ca 85df 85ee)
    ret                        ;85ce    c9      .                  (flow from: 85cd)
    ld (de),a                  ;85cf    12      . 
    inc de                     ;85d0    13      . 
    dec c                      ;85d1    0d      .                  (flow from: 85ce)
    ret nz                     ;85d2    c0      . 
    jr l8603h                  ;85d3    18 2e   . .                (flow from: 85d1)
    ld e,020h                  ;85d5    1e 20   .   
    ld bc,00003h               ;85d7    01 03 00    . . .          (flow from: 85d6)
l85dah:
    ld a,e                     ;85da    7b      {                  (flow from: 85d8)
    call 029a9h                ;85db    cd a9 29    . . )          (flow from: 85da)
    djnz l85dah                ;85de    10 fa   . . 
    dec c                      ;85e0    0d      . 
    jr nz,l85dah               ;85e1    20 f7     .                (flow from: 85d8)
    ret                        ;85e3    c9      . 
    ld a,001h                  ;85e4    3e 01   > .                (flow from: 85e1)
    ld (01ef9h),a              ;85e6    32 f9 1e    2 . . 
    call 01a45h                ;85e9    cd 45 1a    . E . 
    ld a,000h                  ;85ec    3e 00   > . 
    or a                       ;85ee    b7      .                  (flow from: 85ed)
    ld a,013h                  ;85ef    3e 13   > . 
    jp nz,022dah               ;85f1    c2 da 22    . . " 
    call 01ee1h                ;85f4    cd e1 1e    . . . 
    call 00000h                ;85f7    cd 00 00    . . . 
    call 02879h                ;85fa    cd 79 28    . y (          (flow from: 85f8 8605)
    di                         ;85fd    f3      .                  (flow from: 85fb 8601)
    ld e,07eh                  ;85fe    1e 7e   . ~ 
    call 01ee3h                ;8600    cd e3 1e    . . .          (flow from: 85ff)
l8603h:
    ld hl,059e0h               ;8603    21 e0 59    ! . Y          (flow from: 8601)
    ...
    ld bc,l9e00h               ;8614    01 00 9e    . . .          (flow from: 868c)
    call 02521h                ;8617    cd 21 25    . ! %          (flow from: 8614)
...
l8628h:
    ...
    call 01a40h                ;862b    cd 40 1a    . @ .          (flow from: 8629)
    call 02879h                ;862e    cd 79 28    . y (          (flow from: 862d)
    push af                    ;8631    f5      . 
    ld a,00fh                  ;8632    3e 0f   > . 
    call 02c34h                ;8634    cd 34 2c    . 4 , 
    ld a,00fh                  ;8637    3e 0f   > . 
    ld (01f3fh),a              ;8639    32 3f 1f    2 ? .          (flow from: 7cfa 861e 8636)
    pop af                     ;863c    f1      .                  (flow from: 86b9 86bb 86bf 86c2)
    cp 015h                    ;863d    fe 15   . . 
    jr z,l8603h                ;863f    28 c2   ( . 
    cp 004h                    ;8641    fe 04   . .                (flow from: 8640)
    jr nz,l865eh               ;8643    20 19     . 
    ld ix,(024adh)             ;8645    dd 2a ad 24     . * . $    (flow from: 8642)
    ld hl,02d3fh               ;8649    21 3f 2d    ! ? -          (flow from: 8648)
    push hl                    ;864c    e5      . 
    ld bc,02000h               ;864d    01 00 20    . .            (flow from: 864b)
    call 02521h                ;8650    cd 21 25    . ! % 
    pop hl                     ;8653    e1      . 
    call 02378h                ;8654    cd 78 23    . x #          (flow from: 8652)
    ld a,001h                  ;8657    3e 01   > .                (flow from: 8655)
    ld (0205fh),a              ;8659    32 5f 20    2 _   
    jr l8665h                  ;865c    18 07   . .                (flow from: 865a)
l865eh:
    cp 014h                    ;865e    fe 14   . . 
...
l866ch:
    ld b,014h                  ;866c    06 14   . .                (flow from: 865a)
    cp 006h                    ;866e    fe 06   . . 
    jr nz,l8679h               ;8670    20 07     .                (flow from: 866f)
l8672h:
...
l8679h:
    cp 009h                    ;8679    fe 09   . . 
    jr nz,l86a7h               ;867b    20 2a     *                (flow from: 8652 8663 8671 867a 86de 86e5)
l867dh:
    call 01eb5h                ;867d    cd b5 1e    . . .          (flow from: 867c)
    ld de,04040h               ;8680    11 40 40    . @ @ 
    ld a,018h                  ;8683    3e 18   > .                (flow from: 8681)
l8685h:
    call 01e94h                ;8685    cd 94 1e    . . . 
    add a,008h                 ;8688    c6 08   . .                (flow from: 8686)
    cp 0a9h                    ;868a    fe a9   . . 
    jr c,l8685h                ;868c    38 f7   8 .                (flow from: 868b)
    ld hl,050a0h               ;868e    21 a0 50    ! . P 
    call 02835h                ;8691    cd 35 28    . 5 ( 
    ld b,006h                  ;8694    06 06   . .                (flow from: 8693)
    ld hl,(024adh)             ;8696    2a ad 24    * . $          (flow from: 8694 8699)
l8699h:
    call 02488h                ;8699    cd 88 24    . . $          (flow from: 8698)
    djnz l8699h                ;869c    10 fb   . . 
    call 01ea1h                ;869e    cd a1 1e    . . . 
    bit 4,a                    ;86a1    cb 67   . g                (flow from: 86a0)
    jr z,l867dh                ;86a3    28 d8   ( .                (flow from: 86a1 86a6)
l86a5h:
    jr l8628h                  ;86a5    18 81   . .                (flow from: 86a4)
l86a7h:
    cp 007h                    ;86a7    fe 07   . . 
    jr nz,l86b2h               ;86a9    20 07     . 
l86abh:
    call 01each                ;86ab    cd ac 1e    . . .          (flow from: 86a8)
    djnz l86abh                ;86ae    10 fb   . .                (flow from: 8626 8639)
l86b0h:
    jr l8677h                  ;86b0    18 c5   . . 
l86b2h:
    cp 00ah                    ;86b2    fe 0a   . .                (flow from: 02b2 02b5 02b8 02be)
    jr nz,l86e0h               ;86b4    20 2a     * 
l86b6h:
    call 01each                ;86b6    cd ac 1e    . . . 
    ld de,050a0h               ;86b9    11 a0 50    . . P          (flow from: 86b8)
    ld a,0a0h                  ;86bc    3e a0   > .                (flow from: 86bb)
l86beh:
    call 01e94h                ;86be    cd 94 1e    . . . 
    sub 008h                   ;86c1    d6 08   . . 
    cp 010h                    ;86c3    fe 10   . .                (flow from: 864b)
    jr nc,l86beh               ;86c5    30 f7   0 .                (flow from: 86c4)
    ld hl,04040h               ;86c7    21 40 40    ! @ @          (flow from: 86c5)
    call 02835h                ;86ca    cd 35 28    . 5 ( 
    ld b,00dh                  ;86cd    06 0d   . .                (flow from: 86cc)
    ld hl,(024adh)             ;86cf    2a ad 24    * . $ 
l86d2h:
    call 02475h                ;86d2    cd 75 24    . u $          (flow from: 86d1)
    djnz l86d2h                ;86d5    10 fb   . .                (flow from: 86d4)
    call 01ea1h                ;86d7    cd a1 1e    . . .          (flow from: 86d6)
    bit 3,a                    ;86da    cb 5f   . _                (flow from: 873c)
    jr z,l86b6h                ;86dc    28 d8   ( .                (flow from: 86da)
l86deh:
    jr l86a5h                  ;86de    18 c5   . .                (flow from: 86dc)
l86e0h:
    cp 00ch                    ;86e0    fe 0c   . .                (flow from: 86ce)
    jr nz,l86fah               ;86e2    20 16     . 
    ld bc,00001h               ;86e4    01 01 00    . . .          (flow from: 86e3)
...
l870ch:
    ...
    jr nz,l86deh               ;870f    20 cd     .                (flow from: 8690)
    ld hl,05ae0h               ;8711    21 e0 5a    ! . Z          (flow from: 870f)
    ld bc,0203fh               ;8714    01 3f 20    . ?   
    call 02521h                ;8717    cd 21 25    . ! % 
    ld hl,02d3fh               ;871a    21 3f 2d    ! ? -          (flow from: 8718 8724)
    call 027eeh                ;871d    cd ee 27    . . '          (flow from: 871b 871f)
    ld d,000h                  ;8720    16 00   . . 
    ld c,009h                  ;8722    0e 09   . .                (flow from: 8721)
    cp 080h                    ;8724    fe 80   . .                (flow from: 8723)
    jr c,l873bh                ;8726    38 13   8 .                (flow from: 841f 865c 8724 87b2)
    ld hl,01f0fh               ;8728    21 0f 1f    ! . .          (flow from: 8726)
    ld (02079h),hl             ;872b    22 79 20    " y            (flow from: 8729)
    push hl                    ;872e    e5      .                  (flow from: 82bd 83cd 8673 86d7 87bd)
    ld h,d                     ;872f    62      b 
    ld l,a                     ;8730    6f      o                  (flow from: 872e)
    ld de,01209h               ;8731    11 09 12    . . .          (flow from: 8730)
    add hl,hl                  ;8734    29      )                  (flow from: 8733)
    add hl,de                  ;8735    19      . 
    ld a,(hl)                  ;8736    7e      ~                  (flow from: 8734)
    inc hl                     ;8737    23      # 
    ld h,(hl)                  ;8738    66      f                  (flow from: 8736)
    ld l,a                     ;8739    6f      o 
    jp (hl)                    ;873a    e9      .                  (flow from: 8738)
l873bh:
    call 02127h                ;873b    cd 27 21    . ' !          (flow from: 8736 873a)
    call 02485h                ;873e    cd 85 24    . . $ 
    push hl                    ;8741    e5      .                  (flow from: 8a1e)
    ld (02bafh),hl             ;8742    22 af 2b    " . +          (flow from: 8741)
    ld de,02d61h               ;8745    11 61 2d    . a -          (flow from: 8744)
    ld a,(de)                  ;8748    1a      .                  (flow from: 87a9)
    ld c,a                     ;8749    4f      O                  (flow from: 8748)
    inc de                     ;874a    13      .                  (flow from: 85dc)
    call 02cach                ;874b    cd ac 2c    . . , 
    pop hl                     ;874e    e1      .                  (flow from: 874c)
    ld (024adh),hl             ;874f    22 ad 24    " . $          (flow from: 874e)
    ld a,000h                  ;8752    3e 00   > . 
    or a                       ;8754    b7      . 
    jr z,l8763h                ;8755    28 0c   ( .                (flow from: 8753)
    call 02472h                ;8757    cd 72 24    . r $          (flow from: 8756)
    ld (024adh),hl             ;875a    22 ad 24    " . $          (flow from: 8759)
    ld bc,00001h               ;875d    01 01 00    . . . 
    call 02bcah                ;8760    cd ca 2b    . . +          (flow from: 875f)
l8763h:
    ld a,00fh                  ;8763    3e 0f   > .                (flow from: 8761)
    ld (01f3fh),a              ;8765    32 3f 1f    2 ? .          (flow from: 8764)
    xor a                      ;8768    af      . 
    ld (0205fh),a              ;8769    32 5f 20    2 _            (flow from: 7ca7 85ea 8767)
    jp 01f0fh                  ;876c    c3 0f 1f    . . . 
    ld hl,02d40h               ;876f    21 40 2d    ! @ - 
    cp 00dh                    ;8772    fe 0d   . .                (flow from: 8770)
    ret z                      ;8774    c8      .                  (flow from: 8772 8778)
    cp 008h                    ;8775    fe 08   . . 
    jr nz,l8785h               ;8777    20 0c     . 
    ld b,(hl)                  ;8779    46      F 
    dec hl                     ;877a    2b      +                  (flow from: 8772 8778)
    ld a,(hl)                  ;877b    7e      ~                  (flow from: 877a)
    rlca                       ;877c    07      . 
    jr c,l87a6h                ;877d    38 27   8 '                (flow from: 877b)
    ld c,(hl)                  ;877f    4e      N                  (flow from: 8745 876a)
    ld (hl),b                  ;8780    70      p                  (flow from: 877f)
    inc hl                     ;8781    23      # 
    ld (hl),c                  ;8782    71      q                  (flow from: 8780)
    jr l87a6h                  ;8783    18 21   . !                (flow from: 8782)
l8785h:
    cp 00bh                    ;8785    fe 0b   . .                (flow from: 8784)
    jr nz,l8793h               ;8787    20 0a     .                (flow from: 8786)
    ld b,(hl)                  ;8789    46      F 
    inc hl                     ;878a    23      #                  (flow from: 8787)
    ld a,(hl)                  ;878b    7e      ~                  (flow from: 878a)
    and a                      ;878c    a7      . 
    jr z,l87a6h                ;878d    28 17   ( .                (flow from: 878b 8794)
    ld (hl),b                  ;878f    70      p                  (flow from: 878e)
    dec hl                     ;8790    2b      +                  (flow from: 878f)
    ld (hl),a                  ;8791    77      w                  (flow from: 8790)
    ret                        ;8792    c9      .                  (flow from: 8791)
l8793h:
    cp 003h                    ;8793    fe 03   . .                (flow from: 8792)
    jr nz,l87a8h               ;8795    20 11     . 
    ld d,h                     ;8797    54      T                  (flow from: 8796)
    ld e,l                     ;8798    5d      ]                  (flow from: 8797)
    dec hl                     ;8799    2b      +                  (flow from: 8798)
    ld a,(hl)                  ;879a    7e      ~ 
    cp 080h                    ;879b    fe 80   . .                (flow from: 8799)
    jr z,l87a6h                ;879d    28 07   ( . 
l879fh:
    ld a,(de)                  ;879f    1a      . 
    ld (hl),a                  ;87a0    77      w                  (flow from: 879e)
    inc hl                     ;87a1    23      # 
    inc de                     ;87a2    13      .                  (flow from: 87a0)
    or a                       ;87a3    b7      . 
    jr nz,l879fh               ;87a4    20 f9     .                (flow from: 87a0 87a2)
l87a6h:
    inc a                      ;87a6    3c      < 
    ret                        ;87a7    c9      . 
l87a8h:
    cp 005h                    ;87a8    fe 05   . .                (flow from: 87a5)
    jr nz,l87b5h               ;87aa    20 09     .                (flow from: 8815)
    ld hl,028aeh               ;87ac    21 ae 28    ! . ( 
    ld a,0f7h                  ;87af    3e f7   > .                (flow from: 87ad 87c9)
    sub (hl)                   ;87b1    96      . 
    ld (hl),a                  ;87b2    77      w                  (flow from: 85b1)
    jr l87a6h                  ;87b3    18 f1   . . 
l87b5h:
    cp 020h                    ;87b5    fe 20   .                  (flow from: 8728 872b 872d)
    ret c                      ;87b7    d8      .                  (flow from: 87b5)
    ld b,a                     ;87b8    47      G 
    jr nz,l87e0h               ;87b9    20 25     %                (flow from: 87b7)
    ld de,02d3fh               ;87bb    11 3f 2d    . ? -          (flow from: 87b9)
    ld a,(de)                  ;87be    1a      . 
    cp 03bh                    ;87bf    fe 3b   .              ; 
    jr z,l87e0h                ;87c1    28 1d   ( . 
    rlca                       ;87c3    07      .                  (flow from: 87c2)
    jr c,l87e0h                ;87c4    38 1a   8 .                (flow from: 87c3)
    push hl                    ;87c6    e5      .                  (flow from: 87c5)
    sbc hl,de                  ;87c7    ed 52   . R                (flow from: 87c6)
    ld a,00dh                  ;87c9    3e 0d   > .                (flow from: 87c7)
    sub l                      ;87cb    95      . 
    pop hl                     ;87cc    e1      . 
l87cdh:
    jr c,l87e0h                ;87cd    38 11   8 . 
    cp 005h                    ;87cf    fe 05   . .                (flow from: 87ce)
    jr c,l87d5h                ;87d1    38 02   8 . 
    sub 005h                   ;87d3    d6 05   . .                (flow from: 87d2)
l87d5h:
    ld b,a                     ;87d5    47      G 
    inc b                      ;87d6    04      .                  (flow from: 87d4)
    ld c,020h                  ;87d7    0e 20   .   
    call 02521h                ;87d9    cd 21 25    . ! %          (flow from: 87d6)
    ld (hl),001h               ;87dc    36 01   6 .                (flow from: 87db)
    jr l87a6h                  ;87de    18 c6   . .                (flow from: 87dd)
l87e0h:
    ld a,007h                  ;87e0    3e 07   > .                (flow from: 87df)
    or a                       ;87e2    b7      .                  (flow from: 87e1)
    jr z,l87a6h                ;87e3    28 c1   ( . 
    ld a,b                     ;87e5    78      x                  (flow from: 87e2)
l87e6h:
    ex af,af'                  ;87e6    08      .                  (flow from: 87e5)
    ld a,(hl)                  ;87e7    7e      ~                  (flow from: 87e6)
    ex af,af'                  ;87e8    08      . 
    ld (hl),a                  ;87e9    77      w 
    cp 0c5h                    ;87ea    fe c5   . .                (flow from: 87e7)
    ret z                      ;87ec    c8      . 
    cp 0c8h                    ;87ed    fe c8   . .                (flow from: 8811)
    ret z                      ;87ef    c8      .                  (flow from: 87ee)
    cp 0cbh                    ;87f0    fe cb   . .                (flow from: 87ef)
    ret z                      ;87f2    c8      .                  (flow from: 87f0)
    cp 0d7h                    ;87f3    fe d7   . .                (flow from: 87f2)
    ret z                      ;87f5    c8      .                  (flow from: 87f4)
    inc hl                     ;87f6    23      #                  (flow from: 87f5)
    ex af,af'                  ;87f7    08      . 
    or a                       ;87f8    b7      . 
    jr nz,l87e6h               ;87f9    20 eb     .                (flow from: 87f6)
    jr l87a6h                  ;87fb    18 a9   . . 
l87fdh:
    ld ix,02d64h               ;87fd    dd 21 64 2d     . ! d -    (flow from: 87fa)
    ld (ix-002h),001h          ;8801    dd 36 fe 01     . 6 . .    (flow from: 8800)
    ld (ix-001h),037h          ;8805    dd 36 ff 37     . 6 . 7    (flow from: 8804)
    ld b,0ffh                  ;8809    06 ff   . .                (flow from: 8808)
    jr l8810h                  ;880b    18 03   . .                (flow from: 880a)
l880dh:
    call 027edh                ;880d    cd ed 27    . . '          (flow from: 880c)
l8810h:
    call 02ba2h                ;8810    cd a2 2b    . . +          (flow from: 880f)
    or a                       ;8813    b7      .                  (flow from: 8811)
    jr nz,l880dh               ;8814    20 f7     .                (flow from: 8813)
    dec ix                     ;8816    dd 2b   . + 
    jp 0222fh                  ;8818    c3 2f 22    . / "          (flow from: 880b 8814)
    cp 03bh                    ;881b    fe 3b   .              ; 
    jr z,l87fdh                ;881d    28 de   ( . 
    cp 020h                    ;881f    fe 20   .                  (flow from: 881c)
    call nz,027afh             ;8821    c4 af 27    . . ' 
    call 027f5h                ;8824    cd f5 27    . . ' 
    ld de,02d39h               ;8827    11 39 2d    . 9 - 
    ld c,005h                  ;882a    0e 05   . .                (flow from: 8829)
    call 027afh                ;882c    cd af 27    . . '          (flow from: 882b)
    call 027f5h                ;882f    cd f5 27    . . ' 
    ld de,02d25h               ;8832    11 25 2d    . % - 
    ld c,012h                  ;8835    0e 12   . . 
    call 027b3h                ;8837    cd b3 27    . . '          (flow from: 8989)
    jr nz,l8840h               ;883a    20 04     .                (flow from: 8837)
    inc hl                     ;883c    23      #                  (flow from: 883b)
    ld (02d38h),a              ;883d    32 38 2d    2 8 -          (flow from: 883c)
l8840h:
    ld de,02d11h               ;8840    11 11 2d    . . -          (flow from: 883d)
    ld c,012h                  ;8843    0e 12   . .                (flow from: 8842)
    call 027b7h                ;8845    cd b7 27    . . ' 
    call 025cbh                ;8848    cd cb 25    . . %          (flow from: 8847)
    ld hl,02d39h               ;884b    21 39 2d    ! 9 - 
    ld a,(hl)                  ;884e    7e      ~                  (flow from: 884d)
    or a                       ;884f    b7      . 
    jr z,l8863h                ;8850    28 11   ( .                (flow from: 884e)
    push hl                    ;8852    e5      . 
    call 0275fh                ;8853    cd 5f 27    . _ '          (flow from: 8851)
    ld hl,02785h               ;8856    21 85 27    ! . '          (flow from: 8854)
    call 02799h                ;8859    cd 99 27    . . '          (flow from: 8858)
    pop hl                     ;885c    e1      . 
    call 02767h                ;885d    cd 67 27    . g '          (flow from: 885b)
    jp c,022bdh                ;8860    da bd 22    . . " 
l8863h:
    ld (021d9h),a              ;8863    32 d9 21    2 . ! 
    cp 03ah                    ;8866    fe 3a   . :                (flow from: 8864)
    jr c,l886fh                ;8868    38 05   8 . 
    cp 03eh                    ;886a    fe 3e   . >                (flow from: 8869)
    jp c,02241h                ;886c    da 41 22    . A "          (flow from: 8840 886b)
l886fh:
    ld hl,02d25h               ;886f    21 25 2d    ! % - 
    push hl                    ;8872    e5      .                  (flow from: 8871)
    call 0275fh                ;8873    cd 5f 27    . _ ' 
    pop hl                     ;8876    e1      .                  (flow from: 8875)
    ld a,b                     ;8877    78      x 
    dec a                      ;8878    3d      =                  (flow from: 8876)
    jr nz,l88a3h               ;8879    20 28     (                (flow from: 8878)
    ld a,(021d9h)              ;887b    3a d9 21    : . !          (flow from: 887a)
    cp 006h                    ;887e    fe 06   . .                (flow from: 887b)
    jr nz,l8890h               ;8880    20 0e     .                (flow from: 887e)
    ld a,(hl)                  ;8882    7e      ~                  (flow from: 8881)
    sub 02fh                   ;8883    d6 2f   . /                (flow from: 8882)
    cp 004h                    ;8885    fe 04   . .                (flow from: 8884)
l8887h:
    jp nc,022c1h               ;8887    d2 c1 22    . . "          (flow from: 8886)
    or a                       ;888a    b7      . 
    jp z,022c1h                ;888b    ca c1 22    . . "          (flow from: 88ee 8901)
    jr l88a6h                  ;888e    18 16   . . 
l8890h:
    cp 011h                    ;8890    fe 11   . . 
    jr z,l889ch                ;8892    28 08   ( .                (flow from: 8989)
    cp 026h                    ;8894    fe 26   . & 
    jr z,l889ch                ;8896    28 04   ( .                (flow from: 8893)
    cp 031h                    ;8898    fe 31   . 1                (flow from: 8896)
    jr nz,l88a3h               ;889a    20 07     . 
l889ch:
    ld a,(hl)                  ;889c    7e      ~                  (flow from: 8901)
    sub 02fh                   ;889d    d6 2f   . / 
    cp 009h                    ;889f    fe 09   . .                (flow from: 889c)
    jr l8887h                  ;88a1    18 e4   . . 
l88a3h:
    call 02720h                ;88a3    cd 20 27    .   '          (flow from: 88a2)
l88a6h:
    ld (021cbh),a              ;88a6    32 cb 21    2 . !          (flow from: 88a5)
    ld hl,02d11h               ;88a9    21 11 2d    ! . - 
    call 02720h                ;88ac    cd 20 27    .   ' 
    ld (021c3h),a              ;88af    32 c3 21    2 . !          (flow from: 88ae)
    xor a                      ;88b2    af      . 
    ld (02201h),a              ;88b3    32 01 22    2 . "          (flow from: 88b0)
    ld a,000h                  ;88b6    3e 00   > . 
    call 02703h                ;88b8    cd 03 27    . . ' 
    add a,a                    ;88bb    87      . 
    add a,a                    ;88bc    87      .                  (flow from: 88d0)
    ld e,a                     ;88bd    5f      _ 
    ld a,000h                  ;88be    3e 00   > . 
    call 02703h                ;88c0    cd 03 27    . . ' 
    ld d,a                     ;88c3    57      W                  (flow from: 88c1)
    ld b,003h                  ;88c4    06 03   . .                (flow from: 88c3)
l88c6h:
    sla e                      ;88c6    cb 23   . #                (flow from: 88c4)
    rl d                       ;88c8    cb 12   . . 
    djnz l88c6h                ;88ca    10 fa   . .                (flow from: 8989)
    ld a,000h                  ;88cc    3e 00   > .                (flow from: 88cb)
    rla                        ;88ce    17      .                  (flow from: 88cd)
    ld hl,030e1h               ;88cf    21 e1 30    ! . 0          (flow from: 88ce)
    ld bc,002afh               ;88d2    01 af 02    . . .          (flow from: 88d0)
l88d5h:
    inc hl                     ;88d5    23      # 
    ex af,af'                  ;88d6    08      .                  (flow from: 88d3)
l88d7h:
    inc hl                     ;88d7    23      # 
    ex af,af'                  ;88d8    08      . 
    inc hl                     ;88d9    23      #                  (flow from: 88d6)
    inc hl                     ;88da    23      # 
    cpi                        ;88db    ed a1   . . 
    jp po,022d8h               ;88dd    e2 d8 22    . . "          (flow from: 88dc)
    jr nz,l88d5h               ;88e0    20 f3     .                (flow from: 88df)
    ex af,af'                  ;88e2    08      . 
    ld a,(hl)                  ;88e3    7e      ~ 
    inc hl                     ;88e4    23      # 
    cp d                       ;88e5    ba      . 
    ...
    and 0e0h                   ;88e9    e6 e0   . . 
    cp e                       ;88eb    bb      . 
    jr nz,l88d7h               ;88ec    20 e9     .                (flow from: 882e 8888 8899 89ae 8a83)
    dec hl                     ;88ee    2b      +                  (flow from: 88ed)
    dec hl                     ;88ef    2b      +                  (flow from: 88ee)
    dec hl                     ;88f0    2b      +                  (flow from: 88ef)
    ld d,(hl)                  ;88f1    56      V                  (flow from: 88f0)
    dec hl                     ;88f2    2b      + 
    ld e,(hl)                  ;88f3    5e      ^                  (flow from: 88f1)
    ld a,000h                  ;88f4    3e 00   > .                (flow from: 88f3)
    or a                       ;88f6    b7      .                  (flow from: 88f4 88f6)
    jr z,l88fdh                ;88f7    28 04   ( . 
    res 5,d                    ;88f9    cb aa   . .                (flow from: 88f4)
    set 4,d                    ;88fb    cb e2   . .                (flow from: 88fa)
l88fdh:
    call 02526h                ;88fd    cd 26 25    . & %          (flow from: 88fc)
    ld a,(021cbh)              ;8900    3a cb 21    : . ! 
    cp 02ch                    ;8903    fe 2c   . , 
    ld de,02d25h               ;8905    11 25 2d    . % - 
    call nc,025e0h             ;8908    d4 e0 25    . . %          (flow from: 8904)
    ld a,(021c3h)              ;890b    3a c3 21    : . !          (flow from: 890a)
...
l8923h:
    ...
    set 7,b                    ;8927    cb f8   . . 
    set 6,b                    ;8929    cb f0   . .                (flow from: 8926)
    ld (ix+000h),b             ;892b    dd 70 00    . p . 
    inc a                      ;892e    3c      < 
l892fh:
    add a,002h                 ;892f    c6 02   . .                (flow from: 892c)
    ld (02d61h),a              ;8931    32 61 2d    2 a - 
    ret                        ;8934    c9      . 
    push af                    ;8935    f5      .                  (flow from: 8932)
    sub 034h                   ;8936    d6 34   . 4 
    ld e,a                     ;8938    5f      _                  (flow from: 8961)
    ld d,037h                  ;8939    16 37   . 7 
    call 02526h                ;893b    cd 26 25    . & % 
    push bc                    ;893e    c5      . 
    ld hl,02d24h               ;893f    21 24 2d    ! $ -          (flow from: 893d)
    xor a                      ;8942    af      . 
    ld c,a                     ;8943    4f      O                  (flow from: 8941 8947)
l8944h:
    inc hl                     ;8944    23      #                  (flow from: 8943)
    inc c                      ;8945    0c      .                  (flow from: 8944)
    cp (hl)                    ;8946    be      . 
    jr nz,l8944h               ;8947    20 fb     .                (flow from: 8945)
    ld de,02d11h               ;8949    11 11 2d    . . -          (flow from: 8947)
    ld a,(de)                  ;894c    1a      .                  (flow from: 894a)
    or a                       ;894d    b7      . 
    jr z,l8954h                ;894e    28 04   ( . 
    ld (hl),02ch               ;8950    36 2c   6 ,                (flow from: 894c)
    inc hl                     ;8952    23      #                  (flow from: 8950)
    xor a                      ;8953    af      . 
l8954h:
    ex de,hl                   ;8954    eb      .                  (flow from: 8952)
l8955h:
    cp (hl)                    ;8955    be      . 
    jr z,l895eh                ;8956    28 06   ( .                (flow from: 8954)
    ldi                        ;8958    ed a0   . . 
    inc c                      ;895a    0c      .                  (flow from: 8956)
    inc c                      ;895b    0c      . 
    jr l8955h                  ;895c    18 f7   . . 
l895eh:
    ld a,012h                  ;895e    3e 12   > . 
    cp c                       ;8960    b9      . 
    jr c,l89bdh                ;8961    38 5a   8 Z                (flow from: 895d)
    pop bc                     ;8963    c1      .                  (flow from: 895a 8962)
    pop af                     ;8964    f1      . 
    cp 03bh                    ;8965    fe 3b   .              ; 
    jr z,l897eh                ;8967    28 15   ( . 
    ld de,02d25h               ;8969    11 25 2d    . % -          (flow from: 8a96 8a9c)
l896ch:
    ld a,02ch                  ;896c    3e 2c   > ,                (flow from: 896b)
    call 025e0h                ;896e    cd e0 25    . . %          (flow from: 896d)
    ld a,(de)                  ;8971    1a      .                  (flow from: 896e)
    or a                       ;8972    b7      .                  (flow from: 8971)
l8973h:
    jr z,l8923h                ;8973    28 ae   ( . 
    ld (ix+000h),02ch          ;8975    dd 36 00 2c     . 6 . ,    (flow from: 8974)
    inc ix                     ;8979    dd 23   . # 
    inc b                      ;897b    04      .                  (flow from: 8978)
    jr l896ch                  ;897c    18 ee   . . 
l897eh:
    ld hl,02d25h               ;897e    21 25 2d    ! % - 
    call 022cdh                ;8981    cd cd 22    . . "          (flow from: 8980)
    call 02ba2h                ;8984    cd a2 2b    . . +          (flow from: 8983)
    inc hl                     ;8987    23      #                  (flow from: 8986)
    ld a,(hl)                  ;8988    7e      ~                  (flow from: 8987)
    call 02ba2h                ;8989    cd a2 2b    . . +          (flow from: 8988)
    inc hl                     ;898c    23      # 
    ld a,(hl)                  ;898d    7e      ~ 
l898eh:
    call 02ba2h                ;898e    cd a2 2b    . . +          (flow from: 898b)
    inc hl                     ;8991    23      # 
    ld a,(hl)                  ;8992    7e      ~                  (flow from: 825a)
    or a                       ;8993    b7      .                  (flow from: 8992)
    jr nz,l898eh               ;8994    20 f8     .                (flow from: 8993)
    dec hl                     ;8996    2b      + 
    call 022cdh                ;8997    cd cd 22    . . "          (flow from: 8995)
    jr l8973h                  ;899a    18 d7   . .                (flow from: 8999)
    push hl                    ;899c    e5      . 
    push de                    ;899d    d5      .                  (flow from: 899b)
    ld hl,(02a5ah)             ;899e    2a 5a 2a    * Z *          (flow from: 899d)
    add hl,bc                  ;89a1    09      .                  (flow from: 89a0)
    jr c,l89adh                ;89a2    38 09   8 .                (flow from: 89a1)
    ld de,(01b86h)             ;89a4    ed 5b 86 1b     . [ . . 
    sbc hl,de                  ;89a8    ed 52   . R                (flow from: 89a7)
    pop de                     ;89aa    d1      .                  (flow from: 89a8)
    pop hl                     ;89ab    e1      .                  (flow from: 89aa)
    ret c                      ;89ac    d8      .                  (flow from: 89ab)
l89adh:
    ld a,007h                  ;89ad    3e 07   > .                (flow from: 89ac)
    jr l89ceh                  ;89af    18 1d   . . 
    ld a,001h                  ;89b1    3e 01   > .                (flow from: 88f8)
    jr l89ceh                  ;89b3    18 19   . .                (flow from: 89b2 89b9)
    ld a,002h                  ;89b5    3e 02   > .                (flow from: 89b4)
    jr l89ceh                  ;89b7    18 15   . .                (flow from: 89b6)
    ld a,003h                  ;89b9    3e 03   > .                (flow from: 89b8)
    jr l89ceh                  ;89bb    18 11   . .                (flow from: 89b9)
l89bdh:
    ld a,004h                  ;89bd    3e 04   > . 
    jr l89ceh                  ;89bf    18 0d   . .                (flow from: 89bc)
    ld a,(hl)                  ;89c1    7e      ~ 
    cp 027h                    ;89c2    fe 27   . '                (flow from: 89e5)
    ret z                      ;89c4    c8      . 
    cp 022h                    ;89c5    fe 22   . "                (flow from: 89c2)
    ret z                      ;89c7    c8      . 
    ld a,005h                  ;89c8    3e 05   > .                (flow from: 89e5)
    jr l89ceh                  ;89ca    18 02   . . 
    ld a,006h                  ;89cc    3e 06   > . 
l89ceh:
    ex af,af'                  ;89ce    08      .                  (flow from: 8989)
    call 02953h                ;89cf    cd 53 29    . S ) 
    ex af,af'                  ;89d2    08      .                  (flow from: 89d1)
    call 02c34h                ;89d3    cd 34 2c    . 4 ,          (flow from: 89d2)
    call 0251bh                ;89d6    cd 1b 25    . . %          (flow from: 89d5)
    ld hl,(024adh)             ;89d9    2a ad 24    * . $ 
    call 02c6fh                ;89dc    cd 6f 2c    . o ,          (flow from: 89db)
    call z,02475h              ;89df    cc 75 24    . u $ 
    ld (024adh),hl             ;89e2    22 ad 24    " . $          (flow from: 89e1)
    jp 01f2eh                  ;89e5    c3 2e 1f    . . .          (flow from: 89e4)
    ld hl,04000h               ;89e8    21 00 40    ! . @          (flow from: 7716)
    call 02835h                ;89eb    cd 35 28    . 5 ( 
    cp 009h                    ;89ee    fe 09   . . 
    jr nz,l8a00h               ;89f0    20 0e     . 
    push af                    ;89f2    f5      .                  (flow from: 7804)
    ld hl,02321h               ;89f3    21 21 23    ! ! # 
    call 02317h                ;89f6    cd 17 23    . . # 
    ld hl,02321h               ;89f9    21 21 23    ! ! #          (flow from: 89f7)
    ld (02300h),hl             ;89fc    22 00 23    " . #          (flow from: 89f9)
    pop af                     ;89ff    f1      .                  (flow from: 89fc)
l8a00h:
    ld hl,02de1h               ;8a00    21 e1 2d    ! . -          (flow from: 89ff)
l8a03h:
    bit 7,(hl)                 ;8a03    cb 7e   . ~ 
    inc hl                     ;8a05    23      # 
    jr z,l8a03h                ;8a06    28 fb   ( .                (flow from: 8a02 8a04)
    dec a                      ;8a08    3d      = 
    jr nz,l8a03h               ;8a09    20 f8     .                (flow from: 8749)
l8a0bh:
    ld a,(hl)                  ;8a0b    7e      ~ 
    call 029a7h                ;8a0c    cd a7 29    . . )          (flow from: 8a09)
    bit 7,(hl)                 ;8a0f    cb 7e   . ~                (flow from: 8a1d)
    inc hl                     ;8a11    23      # 
    jr z,l8a0bh                ;8a12    28 f7   ( .                (flow from: 8a0c 8a0f)
    ret                        ;8a14    c9      . 
    ld d,e                     ;8a15    53      S                  (flow from: 8749)
    ld a,c                     ;8a16    79      y 
    ld l,l                     ;8a17    6d      m 
    ld h,d                     ;8a18    62      b                  (flow from: 8961)
    ld l,a                     ;8a19    6f      o 
    call pe,sub_6821h          ;8a1a    ec 21 68    . ! h 
    ld a,011h                  ;8a1d    3e 11   > .                (flow from: 8a1c)
    ld l,b                     ;8a1f    68      h 
    ld a,0e5h                  ;8a20    3e e5   > . 
    xor a                      ;8a22    af      .                  (flow from: 8a21)
    sbc hl,de                  ;8a23    ed 52   . R 
    pop hl                     ;8a25    e1      .                  (flow from: 8a24)
    ret c                      ;8a26    d8      .                  (flow from: 8a25)
    ex de,hl                   ;8a27    eb      . 
    ret                        ;8a28    c9      . 
    exx                        ;8a29    d9      .                  (flow from: 8a26)
    push ix                    ;8a2a    dd e5   . .                (flow from: 8a29)
    call 02327h                ;8a2c    cd 27 23    . ' #          (flow from: 8a2a)
    ld b,h                     ;8a2f    44      D                  (flow from: 7c78)
    ld c,l                     ;8a30    4d      M                  (flow from: 8a2f)
    pop hl                     ;8a31    e1      .                  (flow from: 8a30)
    push hl                    ;8a32    e5      . 
    xor a                      ;8a33    af      . 
    sbc hl,bc                  ;8a34    ed 42   . B                (flow from: 8a31)
    pop hl                     ;8a36    e1      . 
    jr c,l8a3ch                ;8a37    38 03   8 . 
    ex de,hl                   ;8a39    eb      .                  (flow from: 8a35)
    sbc hl,de                  ;8a3a    ed 52   . R                (flow from: 8a39)
l8a3ch:
    exx                        ;8a3c    d9      .                  (flow from: 8a3a)
    ret                        ;8a3d    c9      .                  (flow from: 8a3c)
    inc ix                     ;8a3e    dd 23   . #                (flow from: 8a3d)
    ...
    push hl                    ;8a6c    e5      .                  (flow from: 7e17)
    ld bc,02000h               ;8a6d    01 00 20    . .   
    call 02521h                ;8a70    cd 21 25    . ! %          (flow from: 8a6f)
    pop hl                     ;8a73    e1      .                  (flow from: 8078)
    ld a,(ix+000h)             ;8a74    dd 7e 00    . ~ .          (flow from: 8a73)
    dec a                      ;8a77    3d      = 
    jr nz,l8a8fh               ;8a78    20 15     . 
    ld a,(ix+001h)             ;8a7a    dd 7e 01    . ~ .          (flow from: 8a79)
    cp 037h                    ;8a7d    fe 37   . 7                (flow from: 8a7c)
    jr nz,l8a8fh               ;8a7f    20 0e     .                (flow from: 8a7e)
l8a81h:
    ld a,(ix+002h)             ;8a81    dd 7e 02    . ~ .          (flow from: 8a80)
    cp 0c0h                    ;8a84    fe c0   . . 
    ld (hl),001h               ;8a86    36 01   6 .                (flow from: 8901)
    ret nc                     ;8a88    d0      . 
    inc ix                     ;8a89    dd 23   . # 
    ld (hl),a                  ;8a8b    77      w 
    inc hl                     ;8a8c    23      # 
    jr l8a81h                  ;8a8d    18 f2   . .                (flow from: 8989)
l8a8fh:
    ...
    pop hl                     ;8a9e    e1      . 
    ld b,009h                  ;8a9f    06 09   . .                (flow from: 8975)
    call 02514h                ;8aa1    cd 14 25    . . %          (flow from: 8aa0 8aa1)
l8aa4h:
    push hl                    ;8aa4    e5      . 
